c 1_extract_space_and_time_data.f

c This program is extracting any valid data from the stations
c   that are in your SnowModel domain.

      implicit none

      integer nstns,nstns_max,ndays_archive,nfiles,nfiles_max,
     &  ndays_archive_max,nyears,nmonts

      parameter (nstns_max=5000)
      parameter (nfiles_max=5000)
      parameter (ndays_archive_max=30000)

      double precision x(nstns_max),y(nstns_max)
      real elev(nstns_max)

      integer id1(nstns_max),id2(nstns_max),id3(nstns_max)

      real swed(nfiles_max,ndays_archive_max)
      real snod(nfiles_max,ndays_archive_max)
      real sden(nfiles_max,ndays_archive_max)
      real tair(nfiles_max,ndays_archive_max)

      real swed_tmp(nfiles_max,ndays_archive_max)
      real snod_tmp(nfiles_max,ndays_archive_max)
      real sden_tmp(nfiles_max,ndays_archive_max)
      real tair_tmp(nfiles_max,ndays_archive_max)

      real undef

      character*4 state(nstns_max)
      character*8 id(nstns_max)

      character*120 path
      integer trailing_blanks,i_len

      integer maxiter_daily,ioffset_daily,iyr_start,imo_start,
     &  idy_start,iyr_end,imo_end,idy_end,ioptn,julian_end,
     &  julian_start,iyr_archive_start,imo_archive_start,
     &  idy_archive_start,k,kk,kkk,kkkk,nodata_flag,nsnotel

c Read in the SNOTEL path.
      open (26,file='../0_define_run_info/snotel_archive_path.dat')
      read (26,92) path
   92 format (a120)
...
c Define the require temporal domain.  Note that you can't really
c   use the same info from the /met/xxxx/4_*/ directory, because
c   the met and SNOTEL archives might start on different dates.
c   The start and end dates should be the same though.

c This is the begining date of the SNOTEL / SNOW COURSE archive.
      iyr_archive_start = 1980
      imo_archive_start = 1
      idy_archive_start = 1

c Open the SnowModel temporal domain definition file.
      open (81,file=
     &  '../0_define_run_info/sm_temporal_domain.dat')
...
c Calculate the number of time steps in this simulation.
...
c Save this information to a file.
      open (51,file='snotel_maxiter_ioffset.dat')

      write (51,81) iyr_start
      write (51,82) imo_start
      write (51,83) idy_start
      ...
      write (51,93) maxiter_daily
      write (51,94) ioffset_daily
...
c Calculate the number of years and months in the simulation and
c   save that information to a file.
      nyears = iyr_end - iyr_start

c This gives 0 months for complete years, and the number of months
c   in partial years.
      nmonts = imo_end - imo_start
      nmonts = nmonts + 1

      open (52,file='nyears_in_simulation.dat')
      write (52,95) nyears
      write (52,95) nmonts
...
c Read in the number of SNOTEL stations in your SnowModel domain.
      open (73,file='../2_extract_space/n_snotel_stations.dat')
      read (73,*) nsnotel

c Save some info for your grads plots.
      open (53,file='plot_info.dat')
      write (53,96) maxiter_daily
      write (53,96) nstns
      write (53,96) iyr_start

      if (nmonts.gt.0) then
        write (53,96) iyr_end + 1
      else
        write (53,96) iyr_end
      endif
...
c Read in the station info for the stations that are in your
c   SM domain.
      open (41,file='../2_extract_space/stations_in_your_sm_domain.dat')
      do k=1,nstns
        read (41,91)
...
c Read in the total number of stations in the archive.
      open (72,file=
     &  path(1:i_len)//'6_stn_table/total_number_of_stations.dat')
      read (72,*) nfiles

c Read in the number of time records in the SNOTEL archive.
      open (72,file=
     &  path(1:i_len)//'5_mk_grads/ndays_in_archive.dat')
      read (72,*) ndays_archive

c Open and read in the obs data.
      open (21,file=
     &  path(1:i_len)//'5_mk_grads/snotel_snocourse.gdat',
     &  form='unformatted',access='direct',recl=4*4*nfiles)

      do kk=1,ndays_archive
        read (21,rec=kk)
     &    (swed(k,kk),k=1,nfiles),
     &    (snod(k,kk),k=1,nfiles),
     &    (sden(k,kk),k=1,nfiles),
     &    (tair(k,kk),k=1,nfiles)
      enddo

c First extract the stns and times of interest for this SM run,
c   out of the archive.
      do kk=1,maxiter_daily
        kkkk = kk + ioffset_daily
        do k=1,nstns
          kkk = id1(k)
          swed_tmp(k,kk) = swed(kkk,kkkk)
          snod_tmp(k,kk) = snod(kkk,kkkk)
          sden_tmp(k,kk) = sden(kkk,kkkk)
          tair_tmp(k,kk) = tair(kkk,kkkk)
        enddo
      enddo

c Save these data so you can look at it with a plotting script
c   to see what you have.
      open (61,file='snotel_snocourse_sm_domain.gdat',
     &  form='unformatted',access='direct',recl=4*4*nstns,
     &  status='replace')

      do kk=1,maxiter_daily
        write (61,rec=kk)
     &    (swed_tmp(k,kk),k=1,nstns),
     &    (snod_tmp(k,kk),k=1,nstns),
     &    (sden_tmp(k,kk),k=1,nstns),
     &    (tair_tmp(k,kk),k=1,nstns)
      enddo

c Look through these data to see if any of the stations have no
c   data for the period of interest.  Here I am just looking at
c   the swe data.  Save the station counts with valid data to
c   a simple output file.
      open (31,file='stations_w-valid_data.dat')
      do k=1,nstns
        nodata_flag = 0
        do kk=1,maxiter_daily
          if (swed_tmp(k,kk).ne.undef) then
            nodata_flag = 1
          endif
c Also check to see if there is any tair data.  This fixes the
c   problem of having no swed data but still having tair data.
          if (tair_tmp(k,kk).ne.undef) then
            nodata_flag = 1
          endif
        enddo
        if (nodata_flag.eq.0) then
          print *, 'no swe data for stn = ',k
          write (31,95) nint(undef)
        elseif (nodata_flag.eq.1) then
          write (31,95) k
        endif
      enddo
...
c Make the corresponding .ctl file.
      call make_ctl_file (nstns,maxiter_daily,iyr_start)
...
c   quotes, and they cannot be more than 15 characters long.
     ...
     &  'sden     0  0 snow density (kg/m^3)',
     &  'tair     0  0 air temperature (deg C)'/

      open (71,file='snotel_snocourse_sm_domain.ctl')
   ...
   51 format ('DSET ^snotel_snocourse_sm_domain.gdat')
