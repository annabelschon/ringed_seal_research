...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  multilayer_output_fname
      ...
      integer idy_minus1,imo_minus1,iyr_minus1,julian,ioptn
      real xhr_minus1
...
c Note that setting this date-time stamp to be the last time step
c   in the model simulation year is required if you are running
c   the model with irun_data_assim = 1 for more than one year.
c   Check that here (don't worry about the one-year part).
      if (irun_data_assim.eq.1) then

c Find the date-time of the last time step of the first simulation
c   year.  Note that this check only works if xhour_init is 3.0 or
c   12.0.
        if (xhour_init.eq.12.0) then

c First find the Julian date of the model start date.
          ioptn = 3
          call calndr (ioptn,iday_init,imonth_init,iyear_init,julian)

c Then find the month and day that is one day before the start date.
          ioptn = 4
          call calndr (ioptn,idy_minus1,imo_minus1,iyr_minus1,julian-1)

c Define the hour card.
          xhr_minus1 = xhour_init

        elseif (xhour_init.eq.3.0) then

          xhr_minus1 = 0.0
          idy_minus1 = iday_init
          imo_minus1 = imonth_init

        else

          print *, 'This check only works if the model is being'
          print *, 'run with xhour_init = 3.0 (dt == 3 hours) or'
          print *, '12.0 (dt == 1 day).'
          stop

        endif

c Do the date-time check.
        if (imo_minus1.ne.iclear_mn .or. idy_minus1.ne.iclear_dy .or.
     &      xhr_minus1.ne.xclear_hr) then

          write (cmo,'(i2.2)') imo_minus1
          write (cdy,'(i2.2)') idy_minus1
          write (chr,'(i2.2)') nint(xhr_minus1)

          print *, 'In the snowmodel.par file, izero_snow_date'
          print *, '  must be the last time step in the model'
          print *, '  simulation year if you are running the model'
          print *, '  with irun_data_assim = 1.  It looks like'
          print *, '  izero_snow_date should be: ',cmo,cdy,chr
          print *
          stop
...
c     parameter (max_obs_dates=18)
        if (icorr_factor_loop.eq.1) then
          open (unit=61,file='swed_assim/swed_obs.dat')
          read(61,*) nyears
          nobs_total = 0
          do nyear=1,nyears
            read(61,*) nobs_dates
...
c               read(61,*) iiyr,iimo,iidy
                ...
                read(61,*) nstns
                do k=1,nstns
c                 read(61,*) obsid(k),xstn(k),ystn(k),swed_obs(k)
                  read(61,*)
                enddo
              enddo
            endif
...
c       print *
c       print *,'You are running the large-domain Barnes oi scheme'
c       print *,'  This requires:'
c       print *,'  1) no missing data for the fields of interest'
c       print *,'  2) no missing stations during the simulation'
c       print *,'  3) met file must list stations in the same order'
c       print *,'  4) the number of nearest stns used is 9 or less'
c       print *,'  5)  **** no error checking for this is done ****'
c       print *
c       print *,'Generating nearest-station index.  Be patient.'
c       print *
