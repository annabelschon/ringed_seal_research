...
c Variables.
      real prec_sum(nx,ny,nyears)
      real rpre_sum(nx,ny,nyears)
      real spre_sum(nx,ny,nyears)

      real snod_max(nx,ny,nyears)
      real snod_max_dos(nx,ny,nyears)
      real snod_max_doy(nx,ny,nyears)
      real swed_max(nx,ny,nyears)

      real tair_ave(nx,ny,nyears)

      real snow_onset_dos(nx,ny,nyears)
      real snow_onset_doy(nx,ny,nyears)
      real snow_free_dos(nx,ny,nyears)
      real snow_free_doy(nx,ny,nyears)
      real core_snow_days(nx,ny,nyears)

      real snow_first_dos(nx,ny,nyears)
      real snow_first_doy(nx,ny,nyears)
      real snow_last_dos(nx,ny,nyears)
      real snow_last_doy(nx,ny,nyears)
      real total_snow_days(nx,ny,nyears)

      real snod_median(nx,ny,nyears)

      real ros_days(nx,ny,nyears)

c Averages.
      real a_prec_sum(nx,ny)
      real a_rpre_sum(nx,ny)
      real a_spre_sum(nx,ny)

      real a_snod_max(nx,ny)
      real a_snod_max_dos(nx,ny)
      real a_snod_max_doy(nx,ny)
      real a_swed_max(nx,ny)

      real a_tair_ave(nx,ny)

      real a_snow_onset_dos(nx,ny)
      real a_snow_onset_doy(nx,ny)
      real a_snow_free_dos(nx,ny)
      real a_snow_free_doy(nx,ny)
      real a_core_snow_days(nx,ny)

      real a_snow_first_dos(nx,ny)
      real a_snow_first_doy(nx,ny)
      real a_snow_last_dos(nx,ny)
      real a_snow_last_doy(nx,ny)
      real a_total_snow_days(nx,ny)

      real a_snod_median(nx,ny)

      real a_ros_days(nx,ny)

c Slopes.
      real s_prec_sum(nx,ny)
      real s_rpre_sum(nx,ny)
      real s_spre_sum(nx,ny)

      real s_snod_max(nx,ny)
      real s_snod_max_dos(nx,ny)
      real s_snod_max_doy(nx,ny)
      real s_swed_max(nx,ny)

      real s_tair_ave(nx,ny)

      real s_snow_onset_dos(nx,ny)
      real s_snow_onset_doy(nx,ny)
      real s_snow_free_dos(nx,ny)
      real s_snow_free_doy(nx,ny)
      real s_core_snow_days(nx,ny)

      real s_snow_first_dos(nx,ny)
      real s_snow_first_doy(nx,ny)
      real s_snow_last_dos(nx,ny)
      real s_snow_last_doy(nx,ny)
      real s_total_snow_days(nx,ny)

      real s_snod_median(nx,ny)

      real s_ros_days(nx,ny)

c Domain-averaged trends.
      real t_prec_sum(5,nyears)
      real t_rpre_sum(5,nyears)
      real t_spre_sum(5,nyears)

      real t_snod_max(5,nyears)
      real t_snod_max_dos(5,nyears)
      real t_snod_max_doy(5,nyears)
      real t_swed_max(5,nyears)

      real t_tair_ave(5,nyears)

      real t_snow_onset_dos(5,nyears)
      real t_snow_onset_doy(5,nyears)
      real t_snow_free_dos(5,nyears)
      real t_snow_free_doy(5,nyears)
      real t_core_snow_days(5,nyears)

      real t_snow_first_dos(5,nyears)
      real t_snow_first_doy(5,nyears)
      real t_snow_last_dos(5,nyears)
      real t_snow_last_doy(5,nyears)
      real t_total_snow_days(5,nyears)

      real t_snod_median(5,nyears)

      real t_ros_days(5,nyears)
...
c Read in all of the data.
      do nyear=1,nyears
        ...
        read (21,rec=nyear)
     &    ((prec_sum(i,j,nyear),i=1,nx),j=1,ny),
     &    ((rpre_sum(i,j,nyear),i=1,nx),j=1,ny),
     &    ((spre_sum(i,j,nyear),i=1,nx),j=1,ny),

     &    ((snod_max(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snod_max_dos(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snod_max_doy(i,j,nyear),i=1,nx),j=1,ny),
     &    ((swed_max(i,j,nyear),i=1,nx),j=1,ny),

     &    ((tair_ave(i,j,nyear),i=1,nx),j=1,ny),

     &    ((snow_onset_dos(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_onset_doy(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_free_dos(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_free_doy(i,j,nyear),i=1,nx),j=1,ny),
     &    ((core_snow_days(i,j,nyear),i=1,nx),j=1,ny),

     &    ((snow_first_dos(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_first_doy(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_last_dos(i,j,nyear),i=1,nx),j=1,ny),
     &    ((snow_last_doy(i,j,nyear),i=1,nx),j=1,ny),
     &    ((total_snow_days(i,j,nyear),i=1,nx),j=1,ny),

     &    ((snod_median(i,j,nyear),i=1,nx),j=1,ny),

     &    ((ros_days(i,j,nyear),i=1,nx),j=1,ny)
      enddo

c Calculate the trends.
      print *,'calculating trends'
      call linear1(nx,ny,nyears,prec_sum,s_prec_sum,a_prec_sum)
      call linear1(nx,ny,nyears,rpre_sum,s_rpre_sum,a_rpre_sum)
      call linear1(nx,ny,nyears,spre_sum,s_spre_sum,a_spre_sum)

      call linear1(nx,ny,nyears,snod_max,s_snod_max,a_snod_max)
      call linear1(nx,ny,nyears,snod_max_dos,s_snod_max_dos,
     &  a_snod_max_dos)
      call linear1(nx,ny,nyears,swed_max,s_swed_max,a_swed_max)

      call linear1(nx,ny,nyears,tair_ave,s_tair_ave,a_tair_ave)

      call linear1(nx,ny,nyears,snow_onset_dos,s_snow_onset_dos,
     &  a_snow_onset_dos)
      call linear1(nx,ny,nyears,snow_free_dos,s_snow_free_dos,
     &  a_snow_free_dos)
      call linear1(nx,ny,nyears,core_snow_days,s_core_snow_days,
     &  a_core_snow_days)

      call linear1(nx,ny,nyears,snow_first_dos,s_snow_first_dos,
     &  a_snow_first_dos)
      call linear1(nx,ny,nyears,snow_last_dos,s_snow_last_dos,
     &  a_snow_last_dos)
      call linear1(nx,ny,nyears,total_snow_days,s_total_snow_days,
     &  a_total_snow_days)

      call linear1(nx,ny,nyears,snod_median,s_snod_median,a_snod_median)

      call linear1(nx,ny,nyears,ros_days,s_ros_days,a_ros_days)

c Convert these "dos" values to "doy" values.  The aves (a_*)
c   need to be corrected.  The slopes (s_*) should be the same.

c Convert the averages.  Note that if the results of this crosses
c   over the end-of-year boundary, then it will create problems
c   with the trend analyses and plots.  We will deal with that
c   if and when that happens.
      call dos_to_doy (nx,ny,imo_start,idy_start,
     &  a_snod_max_dos,a_snod_max_doy)

      call dos_to_doy (nx,ny,imo_start,idy_start,
     &  a_snow_onset_dos,a_snow_onset_doy)

      call dos_to_doy (nx,ny,imo_start,idy_start,
     &  a_snow_free_dos,a_snow_free_doy)

      call dos_to_doy (nx,ny,imo_start,idy_start,
     &  a_snow_first_dos,a_snow_first_doy)

      call dos_to_doy (nx,ny,imo_start,idy_start,
     &  a_snow_last_dos,a_snow_last_doy)

c Copy the slopes.
      do j=1,ny
        do i=1,nx
          s_snod_max_doy(i,j) = s_snod_max_dos(i,j)
          s_snow_onset_doy(i,j) = s_snow_onset_dos(i,j)
          s_snow_free_doy(i,j) = s_snow_free_dos(i,j)
          s_snow_first_doy(i,j) = s_snow_first_dos(i,j)
          s_snow_last_doy(i,j) = s_snow_last_dos(i,j)
        enddo
      enddo

c Before saving the data, set glaciers, ice sheets, and oceans to
c   undefined.
      do j=1,ny
        do i=1,nx

          if (tair_ave(i,j,1).eq.undef) then
            a_prec_sum(i,j) = undef
            a_rpre_sum(i,j) = undef
            a_spre_sum(i,j) = undef

            a_snod_max(i,j) = undef
            a_snod_max_dos(i,j) = undef
            a_snod_max_doy(i,j) = undef
            a_swed_max(i,j) = undef

            a_tair_ave(i,j) = undef

            a_snod_median(i,j) = undef

            a_ros_days(i,j) = undef

            s_prec_sum(i,j) = undef
            s_rpre_sum(i,j) = undef
            s_spre_sum(i,j) = undef

            s_snod_max(i,j) = undef
            s_snod_max_dos(i,j) = undef
            s_snod_max_doy(i,j) = undef
            s_swed_max(i,j) = undef

            s_tair_ave(i,j) = undef

            s_snod_median(i,j) = undef

            s_ros_days(i,j) = undef
          endif

          if (core_snow_days(i,j,1).eq.undef) then
            a_snow_onset_dos(i,j) = undef
            a_snow_onset_doy(i,j) = undef
            a_snow_free_dos(i,j) = undef
            a_snow_free_doy(i,j) = undef
            a_core_snow_days(i,j) = undef

            s_snow_onset_dos(i,j) = undef
            s_snow_onset_doy(i,j) = undef
            s_snow_free_dos(i,j) = undef
            s_snow_free_doy(i,j) = undef
            s_core_snow_days(i,j) = undef

            a_snow_first_dos(i,j) = undef
            a_snow_first_doy(i,j) = undef
            a_snow_last_dos(i,j) = undef
            a_snow_last_doy(i,j) = undef
            a_total_snow_days(i,j) = undef

            s_snow_first_dos(i,j) = undef
            s_snow_first_doy(i,j) = undef
            s_snow_last_dos(i,j) = undef
            s_snow_last_doy(i,j) = undef
            s_total_snow_days(i,j) = undef
          endif
...
c Save the averages.
     ...
     &  status='replace')
      ...
      write (31,rec=1) ((a_prec_sum(i,j),i=1,nx),j=1,ny)
      write (31,rec=2) ((a_rpre_sum(i,j),i=1,nx),j=1,ny)
      write (31,rec=3) ((a_spre_sum(i,j),i=1,nx),j=1,ny)

      write (31,rec=4) ((a_snod_max(i,j),i=1,nx),j=1,ny)
      write (31,rec=5) ((a_snod_max_dos(i,j),i=1,nx),j=1,ny)
      write (31,rec=6) ((a_snod_max_doy(i,j),i=1,nx),j=1,ny)
      write (31,rec=7) ((a_swed_max(i,j),i=1,nx),j=1,ny)

      write (31,rec=8) ((a_tair_ave(i,j),i=1,nx),j=1,ny)

      write (31,rec=9) ((a_snow_onset_dos(i,j),i=1,nx),j=1,ny)
      write (31,rec=10) ((a_snow_onset_doy(i,j),i=1,nx),j=1,ny)
      write (31,rec=11) ((a_snow_free_dos(i,j),i=1,nx),j=1,ny)
      write (31,rec=12) ((a_snow_free_doy(i,j),i=1,nx),j=1,ny)
      write (31,rec=13) ((a_core_snow_days(i,j),i=1,nx),j=1,ny)

      write (31,rec=14) ((a_snow_first_dos(i,j),i=1,nx),j=1,ny)
      write (31,rec=15) ((a_snow_first_doy(i,j),i=1,nx),j=1,ny)
      write (31,rec=16) ((a_snow_last_dos(i,j),i=1,nx),j=1,ny)
      write (31,rec=17) ((a_snow_last_doy(i,j),i=1,nx),j=1,ny)
      write (31,rec=18) ((a_total_snow_days(i,j),i=1,nx),j=1,ny)

      write (31,rec=19) ((a_snod_median(i,j),i=1,nx),j=1,ny)

      write (31,rec=20) ((a_ros_days(i,j),i=1,nx),j=1,ny)
...
c Save the slopes.
     ...
     &  status='replace')
      ...
      write (32,rec=1) ((s_prec_sum(i,j),i=1,nx),j=1,ny)
      write (32,rec=2) ((s_rpre_sum(i,j),i=1,nx),j=1,ny)
      write (32,rec=3) ((s_spre_sum(i,j),i=1,nx),j=1,ny)

      write (32,rec=4) ((s_snod_max(i,j),i=1,nx),j=1,ny)
      write (32,rec=5) ((s_snod_max_dos(i,j),i=1,nx),j=1,ny)
      write (32,rec=6) ((s_snod_max_doy(i,j),i=1,nx),j=1,ny)
      write (32,rec=7) ((s_swed_max(i,j),i=1,nx),j=1,ny)

      write (32,rec=8) ((s_tair_ave(i,j),i=1,nx),j=1,ny)

      write (32,rec=9) ((s_snow_onset_dos(i,j),i=1,nx),j=1,ny)
      write (32,rec=10) ((s_snow_onset_doy(i,j),i=1,nx),j=1,ny)
      write (32,rec=11) ((s_snow_free_dos(i,j),i=1,nx),j=1,ny)
      write (32,rec=12) ((s_snow_free_doy(i,j),i=1,nx),j=1,ny)
      write (32,rec=13) ((s_core_snow_days(i,j),i=1,nx),j=1,ny)

      write (32,rec=14) ((s_snow_first_dos(i,j),i=1,nx),j=1,ny)
      write (32,rec=15) ((s_snow_first_doy(i,j),i=1,nx),j=1,ny)
      write (32,rec=16) ((s_snow_last_dos(i,j),i=1,nx),j=1,ny)
      write (32,rec=17) ((s_snow_last_doy(i,j),i=1,nx),j=1,ny)
      write (32,rec=18) ((s_total_snow_days(i,j),i=1,nx),j=1,ny)

      write (32,rec=19) ((s_snod_median(i,j),i=1,nx),j=1,ny)

      write (32,rec=20) ((s_ros_days(i,j),i=1,nx),j=1,ny)
...
c   fits, etc., I will figure out how to fix these calculations.
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  prec_sum,t_prec_sum)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  rpre_sum,t_rpre_sum)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     ...
     &  snod_max,t_snod_max)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snod_max_dos,t_snod_max_dos)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snod_max_doy,t_snod_max_doy)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  swed_max,t_swed_max)
     ...
     &  tair_ave,t_tair_ave)
      ...
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_onset_dos,t_snow_onset_dos)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_onset_doy,t_snow_onset_doy)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_free_dos,t_snow_free_dos)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_free_doy,t_snow_free_doy)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  core_snow_days,t_core_snow_days)

      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_first_dos,t_snow_first_dos)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_first_doy,t_snow_first_doy)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_last_dos,t_snow_last_dos)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snow_last_doy,t_snow_last_doy)
      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  total_snow_days,t_total_snow_days)

      call calc_domain_ave_stats(nx,ny,nyears,tair_ave,undef,
     &  snod_median,t_snod_median)
...
c   i=x=5 = t_var(5,k) = r2_var = r^2 fit to the linear line
     ...
     &  status='replace')
      ...
      do nyear=1,nyears
        write (41,rec=nyear)
     &    (t_prec_sum(i,nyear),i=1,5),
     &    (t_rpre_sum(i,nyear),i=1,5),
     &    (t_spre_sum(i,nyear),i=1,5),

     &    (t_snod_max(i,nyear),i=1,5),
     &    (t_snod_max_dos(i,nyear),i=1,5),
     &    (t_snod_max_doy(i,nyear),i=1,5),
     &    (t_swed_max(i,nyear),i=1,5),

     &    (t_tair_ave(i,nyear),i=1,5),

     &    (t_snow_onset_dos(i,nyear),i=1,5),
     &    (t_snow_onset_doy(i,nyear),i=1,5),
     &    (t_snow_free_dos(i,nyear),i=1,5),
     &    (t_snow_free_doy(i,nyear),i=1,5),
     &    (t_core_snow_days(i,nyear),i=1,5),

     &    (t_snow_first_dos(i,nyear),i=1,5),
     &    (t_snow_first_doy(i,nyear),i=1,5),
     &    (t_snow_last_dos(i,nyear),i=1,5),
     &    (t_snow_last_doy(i,nyear),i=1,5),
     &    (t_total_snow_days(i,nyear),i=1,5),

     &    (t_snod_median(i,nyear),i=1,5),

     &    (t_ros_days(i,nyear),i=1,5)
      enddo
