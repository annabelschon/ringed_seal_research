...
c       write (*,151) iyear,imonth,iday,xhour
c 151   format('                   WORKING ON MODEL TIME =',
c    &    i5,2i4,f6.1)
      endif
...
c Open the observation data file.  It must be closed and reopened
c   if doing a swe assimilation run.
      if (iter.eq.1) then
        close (unit=71)
        open (unit=71,file='extra_met/shortwave.dat')
      endif
...
c   if doing a swe assimilation run.
      if (iter.eq.1) then
        close (unit=72)
        open (unit=72,file='extra_met/longwave.dat')
      endif
