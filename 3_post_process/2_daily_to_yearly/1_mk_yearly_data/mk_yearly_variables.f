...
c   to a .gdat file and it will create an associated .ctl file.
     ...
     &  imo_start,idy_start,irec_start,nndays,nnday

      parameter (nx=304,ny=448)

      parameter (nvars=6)

c     real conc_a1a(nx,ny)
      real spre_sum(nx,ny)
      real snod_1ap(nx,ny)
      real swed_1ap(nx,ny)
      real smlt_s1a(nx,ny)
      ...
      real undef,doy_start_of_sim,doy_31Dec
...
c   'snowcover' subroutine).
        ...
        call calc_sum (nx,ny,path,irec_start,ndays,undef,
     i    'spre.gdat',
     o    spre_sum)

c Sum the blowing snow fluxes through 1 April of each year.  To
c   do this, just send in ndays = the number of days between the
c   start of the simulation year and 1 April.
        nndays = 214
        call calc_sum (nx,ny,path,irec_start,nndays,undef,
     i    'bsfx.gdat',
     o    bsfx_s1a)
...
c Average conc and tair between 1 Sep and 1 Apr.
        call calc_ave (nx,ny,path,irec_start,nndays,undef,
     i    'tair.gdat',
     o    tair_a1a)

c       call calc_ave (nx,ny,path,irec_start,nndays,undef,
c    i    'conc.gdat',
c    o    conc_a1a)

c Pull the swe and snow depth on 1 April of each year.
        nnday = 214
        call var_on_date (nx,ny,path,irec_start,nnday,undef,
     i    'snod.gdat',
     o    snod_1ap)
...
c Save the data.
        write (61,rec=nyear)
c    &    ((conc_a1a(i,j),i=1,nx),j=1,ny),
     &    ((spre_sum(i,j),i=1,nx),j=1,ny),
     &    ((snod_1ap(i,j),i=1,nx),j=1,ny),
     &    ((swed_1ap(i,j),i=1,nx),j=1,ny),
     &    ((smlt_s1a(i,j),i=1,nx),j=1,ny),
     &    ((bsfx_s1a(i,j),i=1,nx),j=1,ny),
     &    ((tair_a1a(i,j),i=1,nx),j=1,ny)
...
c   calculate a median value.
      print *, 'number of grid cells with 0 days of snow =',icount0
      print *, 'number of grid cells with 1 day  of snow =',icount1
      print *
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine var_on_date (nx,ny,path,irec_start,nday,undef,
     i  fname,
     o  var)
      ...
      integer nx,ny,i,j,nday,irec,irec_start
...
c Note, here I am not worrying about leap years.
      irec = irec_start + nday - 1
c     print *, nday,irec
