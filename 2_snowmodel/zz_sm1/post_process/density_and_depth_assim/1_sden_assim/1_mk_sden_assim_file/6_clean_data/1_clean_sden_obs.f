...
c Read in the number of years in the simulation.
      open (52,file=path//'3_extract_time/nyears_in_simulation.dat')
      read (52,*) nyears
      read (52,*) nmonts
...
c Read in the data for this SM run.
      open (21,file=
     &  path//'3_extract_time/snotel_snocourse_sm_domain.gdat',
...
c             print *, nday,irec
              if (sden(k,irec).ne.undef) then
                print *, k,nyear,nday,irec,sden(k,irec)
              endif
              sden(k,irec) = undef
            enddo
...
c Save the data.
      open (41,file='snotel_snocrs_sden.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns,
     &  status='replace')

      do nday=1,ndays
        write (41,rec=nday) (sden(k,nday),k=1,nstns)
      enddo
...
c   quotes, and they cannot be more than 15 characters long.
      data var_string /
     &  'sden     0  0 sden (kg/m^3)'/

      open (71,file='snotel_snocrs_sden.ctl')
   ...
   51 format ('DSET ^snotel_snocrs_sden.gdat')
