...
c Read in the data input and output paths.
      open (71,file='0_input_output_paths.dat')
      read (71,*)
...
c     print_inc = 8.0
      ...
      open (61,file='snowmodel_info.dat')
...
c   starting on 1 Sep, and once a year assimilations.
      ...
      call make_sden_ctl_file (nx,ny,ndays,iyear_init,
     &  output_path(1:i_len_output))

      call make_sden_ratio_ctl_file (nx,ny,nyears,iyear_init+1,
     &  output_path(1:i_len_output))
...
c It also assumes the input and output files are daily files.
     ...
     &  ntimes,irec,irec_start,irec_end,nstns_max,nx_max,ny_max
      ...
      real deltax,deltay,undef,dt,print_inc,beta

      real den_obs(nstns_max)

      real sden(nx_max,ny_max)
      real ratio_sden(nx_max,ny_max)
...
c Open the sden output file from the SnowModel run.
      open (41,file=input_path//'sden.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

c Open the output files.
      open (31,file=output_path//'sden_ratio.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny,
     &  status='replace')

      open (32,file=output_path//'sden_assim.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny,
     &  status='replace')

c Open the density observation data file.
      open (unit=21,file=
     &  '../1_mk_sden_assim_file/9_mk_sden_assim_file/sden_obs.dat')
...
c   for each station, at this time step.
        ...
        do k=1,n_den_pts
          read (21,*) id,xstn(k),ystn(k),den_obs(k)
        enddo
...
c Extract the modeled density at that data write.
        read (41,rec=iobs_rec) ((sden(i,j),i=1,nx),j=1,ny)
...
c   differences.
        if (n_den_pts.gt.0) then
          call DENSITY_DATA_ASSIM(nx,ny,deltax,deltay,xmn,ymn,
     &      xstn,ystn,n_den_pts,den_obs,ratio_sden,sden,beta,
...
c Save the density correction surface.
        write (31,rec=nyear) ((ratio_sden(i,j),i=1,nx),j=1,ny)
...
c Use this ratio_sden array to correct all of the snow density
...
c Read in the original data arrays at this time step.
          read (41,rec=irec) ((sden(i,j),i=1,nx),j=1,ny)
          ...
          do j=1,ny
            do i=1,nx
              if (sden(i,j).ne.undef) then
                sden(i,j) = sden(i,j) * ratio_sden(i,j)
                sden(i,j) = max(50.0,sden(i,j))
                sden(i,j) = min(550.0,sden(i,j))
              endif
            enddo
          enddo

c Save the data.
          write (32,rec=irec) ((sden(i,j),i=1,nx),j=1,ny)
...
c Save a text record of the density assimilation information.
      open (51,file='sden_data_output_notes.txt')
      write (51,*) '***********************************************'
      do k=1,nstns
        write (51,*) 'station =',k
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine make_sden_ctl_file (nx,ny,ndays,iyr_start,
     &  path)
...
c   quotes, and they cannot be more than 15 characters long.
   ...
   51 format ('DSET ',a,'sden_assim.gdat')
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine make_sden_ratio_ctl_file (nx,ny,nyears,iyr_start,
     &  path)
...
c   quotes, and they cannot be more than 15 characters long.
      data var_string /
     &  'sden_ratio     0  0 snow density ratio (0-1)'/

      open (71,file='sden_ratio.ctl')
   ...
   51 format ('DSET ',a,'sden_ratio.gdat')
   52 format ('TITLE xxxxxxxxxxxxxxxxxxxxxxxxxx')
   53 format ('UNDEF -9999.0')
