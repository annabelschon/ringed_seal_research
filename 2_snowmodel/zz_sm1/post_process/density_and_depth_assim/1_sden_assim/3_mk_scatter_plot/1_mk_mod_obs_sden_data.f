c 1_mk_mod_obs_sden_data.f

c Here I am making strings of sden data that has all of the obs
c   and mod values so I can put them in a scatter plot.
     ...
     &  nx_max,ny_max
      ...
      real sden_obs(nstns_max)
      real sden_mod(nstns_max)
      real sden(nx_max,ny_max)

      integer k,iiyr,iimo,iidy,i,j,n_den_pts,icount,
     &  iobs_rec,nyear,nyears,ntimes
...
c Define the path where the assimilated output data are located.
      open (71,file='../2_sden_assim/0_input_output_paths.dat')
      read (71,*)
      read (71,*)
      read (71,98) path
   98 format (a120)
...
c     undef = -9999.0
      ...
      open (61,file='../2_sden_assim/snowmodel_info.dat')
...
c Open the SnowModel input files.
      open (21,file=
     &  path(1:i_len)//'sden_assim.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

c Open the sden observation data file.
      open (31,file=
     &  '../1_mk_sden_assim_file/9_mk_sden_assim_file/'//
     &  'sden_obs.dat')

c Output files.
      open (41,file='sden_obs_mod.dat')

      open (51,file='sden_obs_mod.gdat',
...
c   time.
        read (21,rec=iobs_rec) ((sden(i,j),i=1,nx),j=1,ny)

        read (31,*) n_den_pts
        do k=1,n_den_pts
          icount = icount + 1
          read (31,*) id(k),xstn(k),ystn(k),sden_obs(icount)
...
c Extract the data at the obs grid cell.
          sden_mod(icount) = sden(ii(k),jj(k))

c Save the obs and mod data.
          write (41,91)
     &      k,iiyr,iimo,iidy,sden_obs(icount),sden_mod(icount)
...
c Save the obs and mod data.
      do k=1,nstns_max
        write (51,rec=k) sden_obs(k),sden_mod(k)
      enddo
