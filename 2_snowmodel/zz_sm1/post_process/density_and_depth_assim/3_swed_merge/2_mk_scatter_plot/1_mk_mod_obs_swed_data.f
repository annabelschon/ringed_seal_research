c 1_mk_mod_obs_swed_data.f

c Here I am making strings of swed data that has all of the obs
c   and mod values so I can put them in a scatter plot.

      implicit none

      integer nx,ny,iyear_init,imonth_init,iday_init,nstns_max,
     &  nx_max,ny_max

      parameter (nx_max=10000,ny_max=10000)
      parameter (nstns_max=400)

      real deltax,deltay,undef

      double precision xmn,ymn
      double precision xstn(nstns_max),ystn(nstns_max)

      real swed_obs(nstns_max)

      real swed_mod1(nstns_max)
      real swed_mod2(nstns_max)
      real swed_mod3(nstns_max)

      real swed1(nx_max,ny_max)
      real swed2(nx_max,ny_max)
      real swed3(nx_max,ny_max)

      integer k,iiyr,iimo,iidy,i,j,n_den_pts,icount,o_pos,
     &  iobs_rec,nyear,nyears,ntimes,ntime

      integer ii(nstns_max)
      integer jj(nstns_max)
      integer id(nstns_max)

      character*120 path1
      character*120 path2
      character*120 path3
      integer trailing_blanks,i_len1,i_len2,i_len3
...
c Define the path where the assimilated output data are located.
     ...
     &  '../../1_sden_assim/2_sden_assim/0_input_output_paths.dat')
      ...
      read (71,98) path3
   98 format (a120)
      i_len2 = 120 - trailing_blanks(path2)
      i_len3 = 120 - trailing_blanks(path3)

c Build the wo_assim path (because I didn't save it somewhere to
c   read in like the other paths).
      path1 = path2
      i_len1 = i_len2
      o_pos = i_len1 - 7
      path1(o_pos:o_pos) = 'o'
      print *
      print *, 'path1 = ',path1(1:i_len1)
...
c Open the swed SnowModel input files.
      open (21,file=
     &  path1(1:i_len1)//'swed.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

      open (22,file=
     &  path2(1:i_len2)//'swed.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

      open (23,file=
     &  path3(1:i_len3)//'swed_assim.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

c Open the swed observation data file.
      open (31,file='../../../../swe_assim/mk_swed_assim_file/'//
     &  '9_mk_swe_assim_file/swe_obs.dat')

c Output files.
      open (41,file='swed_obs_mod1.dat')
      open (42,file='swed_obs_mod2.dat')
      open (43,file='swed_obs_mod3.dat')

      open (51,file='swed_obs_mod1.gdat',
     &  form='unformatted',access='direct',recl=4*2,
     &  status='replace')

      open (52,file='swed_obs_mod2.gdat',
     &  form='unformatted',access='direct',recl=4*2,
     &  status='replace')

      open (53,file='swed_obs_mod3.gdat',
     &  form='unformatted',access='direct',recl=4*2,
     &  status='replace')

c Process the swed.

c Initialize the write arrays.
      do k=1,nstns_max
        swed_obs(k) = undef
        swed_mod1(k) = undef
        swed_mod2(k) = undef
...
c       print *, ntimes
        ...
        do ntime=1,ntimes
          read (31,*) iiyr,iimo,iidy
c         print *, iiyr,iimo,iidy

c Use the date stamp to calculate the write record for these
c   density observations.
          call get_obs_record(iday_init,imonth_init,iyear_init,
     &      iidy,iimo,iiyr,iobs_rec)
c         print *, nyear,ntimes,iiyr,iimo,iidy,iobs_rec

c Extract the modeled snow depth distribution at that data write
c   time.
          read (21,rec=iobs_rec) ((swed1(i,j),i=1,nx),j=1,ny)
          read (22,rec=iobs_rec) ((swed2(i,j),i=1,nx),j=1,ny)
          read (23,rec=iobs_rec) ((swed3(i,j),i=1,nx),j=1,ny)
...
c         print *, n_den_pts
          ...
          do k=1,n_den_pts
            icount = icount + 1
            read (31,*) id(k),xstn(k),ystn(k),swed_obs(icount)

c Convert the obs coordinates to i,j coordinates.
            ii(k) = 1 + nint((xstn(k) - xmn) / deltax)
            jj(k) = 1 + nint((ystn(k) - ymn) / deltay)

c Extract the data at the obs grid cell.
            swed_mod1(icount) = swed1(ii(k),jj(k))
            swed_mod2(icount) = swed2(ii(k),jj(k))
            swed_mod3(icount) = swed3(ii(k),jj(k))

c Save the obs and mod data.
            write (41,91)
     &        k,iiyr,iimo,iidy,swed_obs(icount),swed_mod1(icount)

            write (42,91)
     &        k,iiyr,iimo,iidy,swed_obs(icount),swed_mod2(icount)

            write (43,91)
     &        k,iiyr,iimo,iidy,swed_obs(icount),swed_mod3(icount)
...
c In the snod assimilation, I didn't assimilate locations where
c   modeled values were less than 0.005 m.  Also remove those
c   from this step.  The divide by 3.0 gives a similar swed
c   threshold.
        if (swed_mod1(k).lt.0.005/3.0) swed_mod1(k) = undef
        if (swed_mod2(k).lt.0.005/3.0) swed_mod2(k) = undef
        if (swed_mod3(k).lt.0.005/3.0) swed_mod3(k) = undef

        write (51,rec=k) swed_obs(k),swed_mod1(k)
        write (52,rec=k) swed_obs(k),swed_mod2(k)
        write (53,rec=k) swed_obs(k),swed_mod3(k)

      enddo

  91  format (4i6,2f10.4)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     &  iidy,iimo,iiyr,julian_end,iobs_rec
...
c   same as the output file data record.
      iobs_rec = julian_end - julian_start + 1
