c extract_variable.f

c There is a Fortran limit on Direct Access records (rec): the
c   rec value can't be larger than (2**31-1), or about 2.2 GB.
c   So, this code deals with that (it defines a record length
c   that allows 'rec' to fit within that limit).
      ...
      real tair_ij_array(nx,ny)
      real snod_ij_array(nx,ny)
      real sden_ij_array(nx,ny)
      real wspd_ij_array(nx,ny)
     ...
     &  idy_end,ioptn,julian_end,julian_start,irec_option,id
      ...
      integer*8 max_size_1,max_size_2,max_size_3,limit_size
...
c Figure out how you have to deal with the rec size limit.
      max_size_1 = nx * ny * nt
      max_size_2 = ny * nt
      max_size_3 = nt
...
c     limit_size = 2**31 - 1
      ...
      if (max_size_1.le.limit_size) then
        irec_option = 1
        nrecs = 1
      elseif (max_size_2.le.limit_size) then
        irec_option = 2
        nrecs = nx
      elseif (max_size_3.le.limit_size) then
        irec_option = 3
        nrecs = nx * ny
      else
        print *, 'You have over 2,147,483,647 time'
        print *, 'records.  Something must be wrong.'
        stop
      endif

      print *, max_size_1,max_size_2,max_size_3,limit_size
      print *, 'irec_option =',irec_option
...
c GPS data file.
      open (31,file='inputs/gps_id_yr_mo_dy_x_y.dat')
...
c Extract that data record from the data-block.
        ...
        elseif (irec_option.eq.2) then

c Find the 1D "x or i" distribution on this date for this location.
          irec = (itime_rec - 1) * ny + jj
c         print *, irec

c Extract the 1D "x or i" distribution on this date for this
c   location.
          read (21,rec=irec) (tair_i_line(i),i=1,nx)
          read (22,rec=irec) (snod_i_line(i),i=1,nx)
          read (23,rec=irec) (sden_i_line(i),i=1,nx)
          read (24,rec=irec) (wspd_i_line(i),i=1,nx)

c Extract the data point within this array.
          tair = tair_i_line(ii)
          snod = snod_i_line(ii)
          sden = sden_i_line(ii)
          wspd = wspd_i_line(ii)

        elseif (irec_option.eq.3) then

c The 2D record is just the time slice in the data block.
          irec = itime_rec
c         print *, irec

c Extract the 2D data distribution on this date.
          read (21,rec=irec) ((tair_ij_array(i,j),i=1,nx),j=1,ny)
          read (22,rec=irec) ((snod_ij_array(i,j),i=1,nx),j=1,ny)
          read (23,rec=irec) ((sden_ij_array(i,j),i=1,nx),j=1,ny)
          read (24,rec=irec) ((wspd_ij_array(i,j),i=1,nx),j=1,ny)

c Extract the data point within this array.
          tair = tair_ij_array(ii,jj)
          snod = snod_ij_array(ii,jj)
          sden = sden_ij_array(ii,jj)
          wspd = wspd_ij_array(ii,jj)
