...
c Here I am converting the daily data to monthly averages.
     ...
     &  imo_plus1,idy_minus1,imo_minus1,julian,i,j
...
c Input files.
c     open (21,file=path//'ice_conc_extracted_domain.gdat',
c    &  form='unformatted',access='direct',recl=4*nx*ny)

c Output file.
c     open (61,file=path_out//'glen_monthly.gdat',
...
c       print *,'processing month ',k

c       if (k.eq.1) then
c         irec_start = 1
c         irec_end = mo_length(k)
c       else
c         irec_start = irec_start + mo_length(k-1)
c         irec_end = irec_end + mo_length(k)
c       endif
...
c       do kk=irec_start,irec_end
c         read (21,rec=kk) ((conc(i,j),i=1,nx),j=1,ny)

c         do j=1,ny
c           do i=1,nx
c             conc_ave(i,j) = conc_ave(i,j) + conc(i,j)
c           enddo
c         enddo
c       enddo
...
c       do j=1,ny
c         do i=1,nx
c           conc_ave(i,j) = conc_ave(i,j) / real(mo_length(k))
c         enddo
c       enddo

c Save the data.
c       write (61,rec=k)
c    &    ((conc_ave(i,j),i=1,nx),j=1,ny)
