c time_test.f
      ...
      integer nt,nobs,nx,ny

      parameter (nt=14610)
      parameter (nobs=14)

      integer julian,k,idy,imo,iyr,irec,irecs_in_day,
     &  iyr_start,imo_start,idy_start,iyr_end,imo_end,ii,jj,
     &  idy_end,ioptn,julian_end,julian_start,id,
     &  itime_rec_days,itime_rec_3hrs,ihr,imn
...
c Read in the data and coordinate information.  Note that you
c   can't have some things in here that don't make sense, like,
c   ihr = 8 and imn = 60; this should be ihr = 9 and imn = 0.
c   It also requires (assumes) the standard SnowModel hour 0
c   stuff [hour 0 is for the previous day (date)].
        read (31,*) id,iyr,imo,idy,ihr,imn,x,y
...
c Find the data-block record.
     ...
     &    (nx * ny) + (jj - 1) * nx + ii
c       irec = (itime_rec - 1) * (nx * ny) + (jj - 1) * nx + ii
c       print *, id,irec
        print *,
     &    id,iyr,imo,idy,ihr,imn,irec,itime_rec_3hrs,itime_rec_days
