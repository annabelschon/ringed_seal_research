c mk_micromet_NLDAS2.f

c Convert the grads input (re)analysis files to the required
c   MicroMet input format.
...
c   grid cells, for example.
     ...
     &  iter,istn,iyr_start,imo_start,idy_start,iyr_end,imo_end,
...
c Define the name of the MicroMet output file.
      open (31,file='mm_3hrly_2017-2018.dat')
c     open (31,file='mm_daily_2000-2003.dat')

c Define whether you want to do the 'drizzle' clipping. See
c   ../5_fix_drizzle_optional/.  This sets any precipitation
c   that is below some threshold to zero, and scales the
c   remaining values to account for what was lost because
c   of the clipped values.  This is really required for any
c   multi-layer SnowModel simulations that are forced with
c   (re)analysis precipitation data, because the 'drizzle'
c   in those atmospheric model products is so prevasive
c   (the result is that no new 'storm' snow layers are ever
c   created in SnowModel).  To do this clipping, set the
c   drizzle_flag = 1.0, turn it off, set drizzle_flag = 0.0.
      drizzle_flag = 0.0
c     drizzle_flag = 1.0
...
c Read in the drizzle adjustment factors.
      if (drizzle_flag.eq.1.0) then

        print *,'I think the smoother also has to be run in steps'
        print *,'5_mk_gdat_optional/ or 6_fix_drizzle_optional/'
        print *,'before doing the drizzle clipping done here.'
        print *,'I have not implemented that in those two options.'
        stop

        open (61,file=
     &    '../6_fix_drizzle_optional/drizzle_storm_clip.gdat',
     &    form='unformatted',access='direct',recl=4*1)

        do iter=1,maxiter
          read (61,rec=iter) clip_mask(iter)
c         print *,iter,clip_mask(iter)
        enddo
      endif
...
c   the data, writing a station count before each day's data write.
...
c We have found that the NLDAS2 precipitation data needs to be
c   smoothed slightly to create uniform precipitation fields
c   (and realistic snow distributions).
c       call smoother9(nx,ny,max_nx,max_ny,prec)
        call smoother9_w_undefs(nx,ny,max_nx,max_ny,undef,prec)

c Process the usable grid points.
        write (31,980) npts
...
c Account for the drizzle parameterization, if desired.  Note
c   that these adjustment factors have been calculated based
c   on snowfall not rainfall, and here they are being applied
c   to both.  If, for some reason, you don't want that, you
c   need to do things differently in 5_fix_drizzle_optional/.
          if (drizzle_flag.eq.1.0) then
            prec(i,j) = clip_mask(iter) * prec(i,j)
          endif

          write (31,990) iyr(iter),imo(iter),idy(iter),
     &      real(ihr(iter)),istn,x_loc(k),y_loc(k),elev(k),
     &      tair(i,j),relh(i,j),wspd(i,j),wdir(i,j),prec(i,j)
        enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine smoother9(nx,ny,max_nx,max_ny,snow)
      ...
      integer i,j,nx,ny
      real snow(max_nx,max_ny)
      real snow_tmp(max_nx,max_ny)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine smoother9_w_undefs(nx,ny,max_nx,max_ny,undef,snow)
