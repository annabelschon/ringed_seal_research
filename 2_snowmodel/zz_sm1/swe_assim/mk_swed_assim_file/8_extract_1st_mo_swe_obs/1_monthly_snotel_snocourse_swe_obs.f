c 1_monthly_snotel_snowcourse_swe_obs.f

c This program is looking at the available SNOTEL and SNOW
c   COURSE swe obs, and extracting the observation locations
...
c   this assumption; SnowModel does not require that.
     ...
     &  nstns_max,ndays_max,n
      ...
      parameter (nyears_max=100)
      parameter (ndays_max=30000)
...
c This is the number of months in a year.
      ...
      real swed(nstns_max,ndays_max)
      real swed_all_obs(nstns_max,ndays_max)
      real swed_used_obs(nstns_max,ndays_max)

      integer nnnyr(nyears_max,nmonths),nnnmo(nyears_max,nmonths),
     &  nnndy(nyears_max,nmonths)

      real swed_tmp(nstns_max,nyears_max,nmonths)
     ...
     &  nmonth,nyear,iundef,month_first,month_last,nmo_diff
      ...
      integer iswed_index(nstns_max,nyears_max,nmonths)
...
c   SWE obs that are available on nndy of the month.
      nndy = 1
...
c   simulation start date.
      open (72,file='../3_extract_time/snotel_maxiter_ioffset.dat')
...
c Read in the number of years in the simulation.
      open (51,file='../3_extract_time/nyears_in_simulation.dat')
      read (51,*) nyears
      read (51,*) nmonts
...
c   program to find out where you are in time.
      open (21,file=
     &  '../7_linear_interp_swe_obs/snotel_snocrs_linear_interp.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns)

c Read in the swe obs.
      do nday=1,ndays
        read (21,rec=nday) (swed(k,nday),k=1,nstns)
      enddo

c For the purposes of creating a swe_obs.dat assimilation file,
c   set any swed=0.0 values to undef, because I don't want to
c   assimilate 0.0 values (because a precipitation correction
c   (multiplication) factor is meaningless in this situation).
      do nday=1,ndays
        do k=1,nstns
          if (swed(k,nday).eq.0.0) swed(k,nday) = undef
        enddo
      enddo
...
c Initialize a swed array to save just the "nndy-Month" observations.
        do nday=1,ndays
          swed_all_obs(k,nday) = undef
          swed_used_obs(k,nday) = undef
        enddo

        do nyr=1,nyears
          do nmo=1,nmonths
            swed_tmp(k,nyr,nmo) = undef
          enddo
        enddo
...
c           print *, nyr,nmo,nnyr,nnmo,nndy,irec,swed(k,irec)
...
c   simulation year.
            if (irec.le.ndays) then
              swed_tmp(k,nyr,nmo) = swed(k,irec)
              swed_all_obs(k,irec) = swed(k,irec)
            else
              swed_tmp(k,nyr,nmo) = undef
              swed_all_obs(k,irec) = undef
            endif
...
c Create a simple index that has 0 for undef swed, and 1 for
c   valid swed values.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (swed_tmp(k,nyear,nmonth).ne.undef) then
              iswed_index(k,nyear,nmonth) = 1
            else
              iswed_index(k,nyear,nmonth) = 0
            endif
          enddo
        enddo
      enddo
...
c Print this information to the screen.
      do nyear=1,nyears
        print *
        do k=1,nstns
          write (*,101) nyear,k,(iswed_index(k,nyear,n),n=1,nmonths)
        enddo
      enddo
...
c   which we do not.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (nmonth.lt.month_first) iswed_index(k,nyear,nmonth) = 0
            if (nmonth.gt.month_last) iswed_index(k,nyear,nmonth) = 0
          enddo
        enddo
      enddo
      ...
      if (nmonts.eq.0) then
        ...
        do nyear=1,nyears
          do k=1,nstns
            icount = 0
            do nmonth=1,nmonths
              if (iswed_index(k,nyear,nmonth).eq.1) icount = icount + 1
            enddo
...
c Use this array info to remove unwanted data from the swed_tmp
c   array.
      do nyear=1,nyears
        do nmonth=1,nmonths
          do k=1,nstns
            if (iswed_index(k,nyear,nmonth).eq.0) then
              swed_tmp(k,nyear,nmonth) = undef
            endif
          enddo
        enddo
      enddo

c Create a grads-writable array that just contains the swed data
c   that will be used in the assimilation.
      do nyear=1,nyears
        do nmonth=1,nmonths
          ...
          do k=1,nstns
            if (swed_tmp(k,nyear,nmonth).ne.undef) then
              swed_used_obs(k,nday) = swed_tmp(k,nyear,nmonth)
            endif
          enddo
        enddo
      enddo
...
c   will be used in the assimilation.
      open (81,file='swe_obs_for_assim.dat')
     ...
     &        nnndy(nyear,nmonth),(swed_tmp(k,nyear,nmonth),k=1,nstns)
          enddo
        enddo
...
c Also make a grads file of these swed obs.
      open (61,file='snotel_swed_obs_used.gdat',
     &  form='unformatted',access='direct',recl=4*2*nstns,
     &  status='replace')

      do nday=1,ndays
        write (61,rec=nday)
     &    (swed_all_obs(k,nday),k=1,nstns),
     &    (swed_used_obs(k,nday),k=1,nstns)
      enddo
...
c   quotes, and they cannot be more than 15 characters long.
      data var_string /
     &  'swedall  0  0 swed depth obs (m)',
     &  'swedused 0  0 swed depth obs (m)'/

      open (71,file='snotel_swed_obs_used.ctl')
   ...
   51 format ('DSET ^snotel_swed_obs_used.gdat')
