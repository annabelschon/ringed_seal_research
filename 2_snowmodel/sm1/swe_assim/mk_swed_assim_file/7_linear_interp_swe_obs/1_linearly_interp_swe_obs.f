c 1_linearly_interp_swe_obs.f

c This program linearly interpolates between swe obs.  This is
c   being done to provide reasonable data for the assimilation
c   at times when there are no observations.

c This program also extends the data forward and backwards in
c   time, by 7-10 days, so that, for example, an obs on 3 April
c   can be used on 1 April.

      implicit none

      integer nstns,ndays_max,nyears,nstns_max
      ...
      real swed_tmp1(ndays_max)
      real swed_tmp2(ndays_max)
      real swed_tmp(nstns_max,ndays_max)

      integer nyear,iyr_start,imo_start,idy_start,irec_start,
     &  nday,nfirst,nsecond,irec,k,ndays,nday2,n_synoptic,kk,kkk,
     &  ndays_total,nmonts

      real undef,x0,x1,xx,wt

      undef = -9999.0

c See the code below for a description of what "n_synoptic" does.
      n_synoptic = 10
...
c Read in the data for this SM run.
      open (21,file='../6_clean_data/snotel_snocrs_swed.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns)

      do nday=1,ndays_total
        read (21,rec=nday) (swed(k,nday),k=1,nstns)
      enddo

c For each year, interpolate daily values between the swe obs.
      do k=1,nstns
        do nyear=1,nyears
...
c Calculate the starting record and number of records in this year.
     ...
     &      irec_start,ndays)
c         print *, k,nyear,irec_start,ndays

c Extract the data string you are going to work with now.
          do nday=1,ndays
            irec = irec_start + nday - 1
c           print *, nday,irec
            swed_tmp1(nday) = swed(k,irec)
          enddo

c Initialize the working array before using it.
          do nday=1,ndays
            swed_tmp2(nday) = undef
          enddo

c Sweep through the year looking for valid swe obs.
          nfirst = 0
          nsecond = 0
          do nday=1,ndays

c This catches the first observation.
            if (swed_tmp1(nday).ne.undef .and. nfirst.eq.0) then
              nfirst = nday
            endif

c This catches the next observation.
            if (swed_tmp1(nday).ne.undef .and. nfirst.ne.0 .and.
     &        nfirst.ne.nday) then
              nsecond = nday
            endif

c This is the case where there are two data points and we want to
c   linearly interpolate between them.
            if (nfirst.ne.0 .and. nsecond.ne.0) then
c             print *, k,nyear,nday,nfirst,nsecond
c             print *, 100.0*swed_tmp1(nfirst),100.0*swed_tmp1(nsecond)

c Don't do the interpolation if the distance between the valid data
c   points are too far apart.  I'm not sure if the following line
c   is correct.
c             if (nsecond-nfirst.le.2*n_synoptic) then

c Define the endpoints.
                x0 = real(nfirst)
                x1 = real(nsecond)

c Calculate the interpolation weights and linearly interpolate
c   the swe values between nfirst and nsecond.
                do nday2=nfirst,nsecond
                  xx = real(nday2)
                  wt = (xx - x0) / (x1 - x0)
                  swed_tmp2(nday2) =
     &              wt * swed_tmp1(nsecond) + (1.0-wt)*swed_tmp1(nfirst)
c                 print *, nday2,100.0*swed_tmp2(nday2)
                enddo

c Set nfirst = nsecond, and look for the next valid data point.
                nfirst = nsecond
                nsecond = 0
...
c Sweep through again looking for the case where there is only a
c   single observed value.
          nfirst = 0
          nsecond = 0
          do nday=1,ndays
...
c This is the case where there was only a single obs in the year.
          if (nfirst.ne.0 .and. nsecond.eq.0) then
c           print *, k,nyear,nfirst,100.0*swed_tmp1(nfirst)
            swed_tmp2(nfirst) = swed_tmp1(nfirst)
          endif

c Sweep through and extend any valid data values "n_synoptic"
c   into any undef areas before and after the valid data points.
c   The assumption here is that any swe observation is valid
c   over the time period of the synoptic cycle that is producing
c   the changes in swe.  Note that this only finds one instance
c   of the sweep forward and then one back.
c Sweep forward in time.
          do nday=2,ndays-n_synoptic
            if (swed_tmp2(nday).eq.undef .and.
     &        swed_tmp2(nday-1).ne.undef) then
              do kk=1,n_synoptic
                kkk = nday + kk - 1
                if (swed_tmp2(kkk).eq.undef) then
                  swed_tmp2(kkk) = swed_tmp2(nday-1)
                endif
              enddo
              goto 101
            endif
...
c Sweep backwards in time.
          do nday=ndays-1,n_synoptic,-1
            if (swed_tmp2(nday).eq.undef .and.
     &        swed_tmp2(nday+1).ne.undef) then
              do kk=1,n_synoptic
                kkk = nday - kk + 1
...
c Place the results in a new swed array.
          do nday=1,ndays
            irec = irec_start + nday - 1
            swed_tmp(k,irec) = swed_tmp2(nday)
          enddo
...
c Save the data.
      open (41,file='snotel_snocrs_linear_interp.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns,
     &  status='replace')

      do nday=1,ndays_total
        write (41,rec=nday) (swed_tmp(k,nday),k=1,nstns)
      enddo

c Make the corresponding .ctl file.
      call make_ctl_file (nstns,ndays_total,iyr_start)
...
c   quotes, and they cannot be more than 15 characters long.
     ...
     &  'swed     0  0 swed depth (m)'/
      ...
      open (71,file='snotel_snocrs_linear_interp.ctl')
   ...
   51 format ('DSET ^snotel_snocrs_linear_interp.gdat')
