...
c This program is being used to delete or fix problems with the
c   input datasets.  The idea here is that you looked at the
c   data in step /3_extract_time/ and you are going to correct
c   any problems you saw here.

      implicit none

      integer nstns,ndays,nyears,nstns_max,ndays_max
      ...
      real tair(nstns_max,ndays_max)

      integer nyear,iyr_start,imo_start,idy_start,irec_start,
     &  nday,irec,k,id1,id2,id3,nmonts
...
c Read in the number of years in the simulation.
      open (52,file='../3_extract_time/nyears_in_simulation.dat')
      read (52,*) nyears
      read (52,*) nmonts
...
c Read in the data for this SM run.
      open (21,file=
     &  '../3_extract_time/snotel_snocourse_sm_domain.gdat',
     &  form='unformatted',access='direct',recl=4*4*nstns)

      do nday=1,ndays
        read (21,rec=nday)
     &    (swed(k,nday),k=1,nstns),
     &    (snod(k,nday),k=1,nstns),
     &    (sden(k,nday),k=1,nstns),
     &    (tair(k,nday),k=1,nstns)
      enddo

c Station info, including whether they are snotel or snowcourse
c   sites.
      open (31,file=
...
c ******************************
c Domain 15A.
c These don't look real to me, so I am fixing them here.
c     swed(1,5550) = swed(1,5551)
c     swed(1,5579) = swed(1,5580)
c     swed(1,2972) = swed(1,2971)
c     do nday=2923,2953
c       swed(1,nday) = 0.0
c     enddo
c ******************************
...
c ******************************
c Domain 20A.
c This value is unrealistically high (0.457m on 5 Dec 2003).
c     swed(9,8496) = undef
c ******************************

c ******************************
c Domain 14A.
c This value is unrealistically high (0.533m on 29 Nov 2001).
c     swed(17,7760) = undef

c Stn 2 is all zeros during this period, so set them to undef.
c     do nday=7848,7917
c       swed(2,nday) = undef
c     enddo
c ******************************
...
c ******************************
c Domain 20D.
c I didn't see any specific issues with the data in this domain.
c ******************************
...
c ******************************
c Domain 15A, 20A, 14A, 20D, 25D.
c There are several snow course stations that have unrealistic
c   1 July and 1 September values (very non-zero).  Here I am
c   setting them to undef.  Note that I am just doing this for
c   the snow course sites, not the SNOTEL stations.
      do k=1,nstns

        read (31,*) id1,id2,id3,state,id,x,y,elev

        if (id3.eq.2) then

          do nyear=1,nyears

c Calculate the starting record in this year.
            call get_irecs (iyr_start,imo_start,idy_start,nyear,
     &        irec_start)
c           print *, k,nyear,irec_start

c Set 1 September values to undef.
            do nday=1,1
              irec = irec_start + nday - 1
c             print *, nday,irec
              if (swed(k,irec).ne.undef .and. swed(k,irec).ne.0.0) then
                print *, k,nyear,nday,irec,100.0*swed(k,irec)
              endif
              swed(k,irec) = undef
            enddo

c Set 1 July values to undef (here I am  accounting for leap
c   years by setting nday to be 304 and 305; if your model run
c   didn't start on 1 Sep, you will have to change this).
            do nday=304,305
              irec = irec_start + nday - 1
c             print *, nday,irec
              if (swed(k,irec).ne.undef .and. swed(k,irec).ne.0.0) then
...
c Save the data.
      open (41,file='snotel_snocrs_swed.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns,
     &  status='replace')

      do nday=1,ndays
        write (41,rec=nday) (swed(k,nday),k=1,nstns)
      enddo
...
c   quotes, and they cannot be more than 15 characters long.
     ...
     &  'swed     0  0 swed depth (m)'/
      ...
      open (71,file='snotel_snocrs_swed.ctl')
   ...
   51 format ('DSET ^snotel_snocrs_swed.gdat')
