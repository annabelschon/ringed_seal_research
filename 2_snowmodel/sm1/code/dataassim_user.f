...
c   to those in the snowmodel.par file).
...
c   /sm/swed_assim/.  And it is assumed to be called: swed_obs.dat.
...
c   file written to /sm/swed_assim/ that provides a summary of
c   the calculations that were done (see unit=77 in the code
c   below for what is written to this file).  Second, there is a
c   copy of the precipitation correction surfaces that were
c   generated.  This is also placed in /sm/swed_assim/, the file
c   is a GrADS file (called corr_factor.gdat), and there is
...
c The file is assumed to be in this directory:
...
c The first line is how many years there are in this SnowModel
c   simulation.  Note that these are SnowModel simulation years.
c   So, for example, year 1 might be 1 Sep 2007 - 31 Aug 2008, and
c   year 2 might be 1 Sep 2008 - 31 Aug 2009, etc.

c   number of years in the simulation
...
c       open(63,file='swed_assim/zack_obs_mask.gdat',
c    &    form='unformatted',access='direct',recl=4*nx*ny)
...
c   mask.  This is only used if local_assim_flag = 1.
     ...
     &  '../swed_obs/2014/barnes/obs.gridded.gdat'
...
c   done).
      fname_sweobs = 'swed_assim/swed_obs.dat'
...
c   at.
        open (unit=77,file='swed_assim/corr_factor.txt')

c Open an output file for the correction factor array.
        open(62,file='swed_assim/corr_factor.gdat',
...
c   original SnowModel output files.
...
c   scheme.
      open (unit=78,file='swed_assim/cf.txt')
      do k=1,nstns
...
c   first loop in SnowModel.
      ...
      open (731,file='swed_assim/cf.txt')
...
c Write out the original stations.
      open (761,file='swed_assim/cf_with_mask.txt')
...
c Read in the new local cf data.
      open (79,file='swed_assim/cf_with_mask.txt')
      read (79,*) nstns2
      do k=1,nstns2
...
c Write the output data to a grads file.
      open(511,file='swed_assim/corr_factor_w-mask.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)
...
c Convert the write interval from seconds to hours or a day.
      ...
      filename = 'swed_assim/corr_factor.ctl'
