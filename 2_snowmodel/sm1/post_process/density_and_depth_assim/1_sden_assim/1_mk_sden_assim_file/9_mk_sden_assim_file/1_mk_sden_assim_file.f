c 1_mk_sden_assim_file.f

c Make a sden assim input file.
...
c Save a sden data assimilation file.
      open (41,file='sden_obs.dat')

c The first line is the number of years in the model run.
      write (41,95) nyears

c Read in the data that will be assimilated.  And write it back
c   out as a sden assim file.
      open (31,file=
     &  '../8_extract_1st_mo_sden_obs/sden_obs_for_assim.dat')
...
c Write the number of obs dates in this year.
        if (nobs_dates.eq.0) then
          ...
          do n=1,nobs_dates
            read (31,*) nyr(n),nmo(n),ndy(n),(sden(k,n),k=1,nstns)
...
c Station id, x, y, sden.
            do k=1,nstns
              if (sden(k,n).ne.undef) then
                if (sden(k,n).ne.0.0) then
                  write (41,98) k,xstn(k),ystn(k),sden(k,n)
                else
                  print *,'sden probably should not be zero; k=',k
                  stop
                endif
              endif
            enddo
