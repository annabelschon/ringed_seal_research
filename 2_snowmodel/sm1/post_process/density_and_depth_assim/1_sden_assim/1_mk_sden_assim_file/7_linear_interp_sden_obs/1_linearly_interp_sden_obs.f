c 1_linearly_interp_sden_obs.f
...
c   can be used on 1 April.
      ...
      real sden_tmp1(ndays_max)
      real sden_tmp2(ndays_max)
      real sden_tmp(nstns_max,ndays_max)
...
c Read in the data for this SM run.
      open (21,file='../6_clean_data/snotel_snocrs_sden.gdat',
...
c           print *, nday,irec
            sden_tmp1(nday) = sden(k,irec)
          enddo

c Initialize the working array before using it.
          do nday=1,ndays
            sden_tmp2(nday) = undef
          enddo
...
c This catches the first observation.
            if (sden_tmp1(nday).ne.undef .and. nfirst.eq.0) then
              nfirst = nday
            endif

c This catches the next observation.
            if (sden_tmp1(nday).ne.undef .and. nfirst.ne.0 .and.
     &        nfirst.ne.nday) then
              nsecond = nday
            endif
...
c             print *, sden_tmp1(nfirst),sden_tmp1(nsecond)
...
c   the swe values between nfirst and nsecond.
                do nday2=nfirst,nsecond
                  ...
                  wt = (xx - x0) / (x1 - x0)
                  sden_tmp2(nday2) =
     &              wt * sden_tmp1(nsecond) + (1.0-wt)*sden_tmp1(nfirst)
c                 print *, nday2,sden_tmp2(nday2)
                enddo
...
c           print *, k,nyear,nfirst,sden_tmp1(nfirst)
            sden_tmp2(nfirst) = sden_tmp1(nfirst)
          endif
...
c Sweep forward in time.
          do nday=2,ndays-n_synoptic
            if (sden_tmp2(nday).eq.undef .and.
     &        sden_tmp2(nday-1).ne.undef) then
              do kk=1,n_synoptic
                kkk = nday + kk - 1
                if (sden_tmp2(kkk).eq.undef) then
                  sden_tmp2(kkk) = sden_tmp2(nday-1)
                endif
              enddo
              goto 101
            endif
...
c Place the results in a new sden array.
          do nday=1,ndays
            irec = irec_start + nday - 1
            sden_tmp(k,irec) = sden_tmp2(nday)
          enddo
...
c Save the data.
     ...
     &  status='replace')
      ...
      do nday=1,ndays_total
        write (41,rec=nday) (sden_tmp(k,nday),k=1,nstns)
      enddo
