...
*    'define obs=100.0*swed_obs.1'
*    'define mod=100.0*swed_mod.1'

*    'define obs=100*swed_obs.1'
*    'define mod=100*swed_mod.1'

     'define obs=swed_obs.1'
     'define mod=swed_mod.1'
...
*************************************
  ...
  'set strsiz 0.22'
   x1 = xmn-0.7
   y1 = (ymn+ymx)/2
  'draw string 'x1' 'y1' SnowModel SWE (m)'

  'set string 1 c 4 0'
  'set strsiz 0.22'
   x1 = (xmn+xmx)/2
   y1 = ymn-0.5
  'draw string 'x1' 'y1' Observed SWE (m)'
...
*'draw title GMU 15A\(DOS = Day Of Simulation = day since 1 Sep)\'
 ...
 'gprint_png fig_files/scatter_swed_obs_mod3'
