...
c   of this program.
      ...
      integer nx,ny,nyears,i,j,k,nvars,nvar,nx_max,ny_max,
     &  max_years,nyear
...
c   years in the simulation.  100 years should be enough.
      parameter (max_years=100) 

      character*15 varname

      integer n_blanks,length,trailing_blanks
...
c These three cards should not be changed.
      character*15 var_string(nvars)
      character*15 varname_15char(nvars)
      integer name_length(nvars)

c This card should not be changed.
      real vars(nx_max,ny_max,nvars)

c This should be copied from "mk_yearly_variables.f".  They
c   cannot be more than 15 characters long.
      data var_string /
     &  'prec_sum',
     &  'rpre_sum',
     &  'spre_sum',
...
c Point to where the SnowModel output data are located.
      parameter (path_in=
     &  '../../daily_to_yearly/1_mk_yearly_data/')
...
c Create the directory the data will be written to.  Note that
c   this only works with the gfortran compiler.
...
c Calculate the number of years in the simulation.
      nyears = iyr_end - iyr_start

      print *
      print *,'# years in the data write = ',nyears
      print *

c Build the year-stamp that corresponds to each model year.
      do nyear=1,nyears
        iyr(nyear) = iyr_start + nyear
      enddo

c This is figuring out how many "_"s are going to be put in the
c   front of the variable name.
      do nvar=1,nvars
        varname = var_string(nvar)
        n_blanks = trailing_blanks(varname)
        length = 15 - n_blanks
        name_length(nvar) = length
        varname_15char(nvar) = blanks(1:n_blanks)//varname(1:length)
      enddo

c Input file.
      open (21,file=path_in//'yearly_variables.gdat',
     &  form='unformatted',access='direct',recl=4*nvars*nx*ny)

c Loop through the years, reading in all the data for each year.
      do nyear=1,nyears

        print *, 'processing year =',nyear

        read (21,rec=nyear) (((vars(i,j,k),i=1,nx),j=1,ny),k=1,nvars)

c Loop through each variable, saving each array as individual
c   .flt files.
        do nvar=1,nvars

c This gives "_____" ahead of the variable name, and all the file
c   names are the same length.  This trick, vars(:,:,nvar), is
c   pulling the 2D array, of variable nvar, out of the 3D array,
c   and passing just that part to the subroutine.
          varname = varname_15char(nvar)
          call flt_outputs (nx,ny,iyr(nyear),path_out,vars(:,:,nvar),
     &      varname,xmn,ymn,deltaxy,undef,run_name,nx_max,
     &      ny_max)

c This gives file names of varying length, depending on the
c   length of the variable name.
c         varname = var_string(nvar)
c         length = name_length(nvar)
c         call flt_outputs (nx,ny,iyr(nyear),path_out,vars(:,:,nvar),
c    &      varname(1:length),xmn,ymn,deltaxy,undef,run_name,nx_max,
c    &      ny_max)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine flt_outputs (nx,ny,iyear,path_out,var,
     &  cvar,xmn,ymn,deltaxy,undef,run_name,nx_max,
     &  ny_max)

      implicit none

      integer i,j,nx,ny,iyear,nx_max,ny_max
      ...
      character*(*) path_out
      character*12 run_name
      character*4 date_string
      character*120 f_p_name1
      character*120 f_p_name2
      character*(*) cvar

c Create the date string.
      write(date_string,'(i4.4)') iyear

c This is the file name with the path included.
      f_p_name1 =
     &  path_out//run_name//'_'//cvar//'_'//date_string//'.flt'

      f_p_name2 =
     &  path_out//run_name//'_'//cvar//'_'//date_string//'.hdr'
