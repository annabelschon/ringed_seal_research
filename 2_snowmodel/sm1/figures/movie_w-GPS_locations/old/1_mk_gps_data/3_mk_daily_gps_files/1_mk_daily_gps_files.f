c 1_mk_daily_gps_files.f

c Here I am going to make a separate location file for each day's
c   worth of GPS location data.

      implicit none

      integer nlines,ndays,nbirds
      ...
      integer k,iyr_start,imo_start,idy_start,ioptn,julian_start,
     &  julian,kkk,kk

      integer id(nlines),imo(nlines),idy(nlines),iyr(nlines),
     &  is(nlines)

      real xxx,yyy,undef
      real x(nlines),y(nlines)

      real xx(nbirds,ndays)
      real yy(nbirds,ndays)
      ...
      open (31,file='../2_reproject/ij_coords.dat')

c Read the times and aea locations.
      read (21,*)
      do k=1,nlines
        read (21,*) id(k),iyr(k),imo(k),idy(k),xxx,yyy
        read (31,*) x(k),y(k)
      enddo
...
c Convert the GPS dates to the "day of simulation".
      do k=1,nlines
        call calndr (ioptn,idy(k),imo(k),iyr(k),julian)
        is(k) = julian - julian_start + 1
c       print *, k,is(k)
      enddo

c Convert the bird numbers to 1-13 (=nbirds).
      do k=1,nlines
        if (id(k).eq.174399) then
          id(k) = 1
        elseif (id(k).eq.174401) then
          id(k) = 2
        elseif (id(k).eq.174402) then
          id(k) = 3
        elseif (id(k).eq.174404) then
          id(k) = 4
        elseif (id(k).eq.174405) then
          id(k) = 5
        elseif (id(k).eq.174406) then
          id(k) = 6
        elseif (id(k).eq.174409) then
          id(k) = 7
        elseif (id(k).eq.174412) then
          id(k) = 8
        elseif (id(k).eq.174413) then
          id(k) = 9
        elseif (id(k).eq.174416) then
          id(k) = 10
        elseif (id(k).eq.174417) then
          id(k) = 11
        elseif (id(k).eq.174419) then
          id(k) = 12
        elseif (id(k).eq.174422) then
          id(k) = 13
        else
          print *,'found new id',k,id(k)
          stop
        endif
      enddo

c     do k=1,nlines
c       print *,id(k),is(k),x(k),y(k)
c     enddo

c Put these in an array to make the processing easier.
...
c Populate the arrays.
      do k=1,nlines
c       print *, k,id(k),is(k),x(k),y(k)
        xx(id(k),is(k)) = x(k)
        yy(id(k),is(k)) = y(k)
      enddo

c Create the gps files.
      do k=ndays,1,-1

c This is opening one file for each day.  The write statement is
c   turning the number into a character, 6 characters long.  So,
c   for example, 1 --> 000001.  And I am putting a "gps_1" in
c   front of it to create the file name.
        write(char_dy,'(i6.6)') k
        fname = 'data/gps_1'//char_dy//'.dat'
        open (51,file=fname)

        do kk=1,nbirds

c Fill this file with gps data from today and all previous days.
          do kkk=k,1,-1

c If you just wanted the previous 4 days, you would do this:
c         do kkk=k,max(1,k-3),-1

            if (xx(kk,kkk).ne.undef) then
              write (51,91) xx(kk,kkk),yy(kk,kkk)
            endif
          enddo
        enddo
        close (51)
