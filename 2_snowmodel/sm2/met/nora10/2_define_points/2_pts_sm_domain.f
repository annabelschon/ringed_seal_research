...
c Point to the coordinate and topography data files.
      ...
      open (22,file=path2//'nora10_topo.txt')
...
c This is not quite right.  The NORA10 domain is not on a
c   recular lat-lon grid.  To use the "thinning" program you
c   will probably have to fix how this is dealt with in that
c   subroutine.
      xlat_start = 50.5542
      if (thin.eq.1.0) then
        print *,'please read the note above'
        stop
      endif
...
c   domain.
      if (thin.eq.0.0) then
        open (23,file=
     &   path1//'process_data/1_topo/outputs/5_ll_sm_corners_final.dat')
        read (23,*) dummy1,yll_lat1
        read (23,*) dummy1,yll_lat2
        read (23,*) dummy1,yul_lat1
        read (23,*) dummy1,yul_lat2
        yll_lat = (yll_lat1 + yll_lat2) / 2.0
        yul_lat = (yul_lat1 + yul_lat2) / 2.0
