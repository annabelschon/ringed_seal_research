c barnes_interp_local_cf_mask.f
...
c Provide the input information.
      ...
      infname = 'anwr_traverse_SWE_2014.dat'
      outfname = 'obs_gridded.gdat'

      undef = -9999.0

c This ifill = 0 is what keeps the influence of each observation
c   from extending over the entire domain.
c     ifill = 1
      ifill = 0
      iobsint = 0
...
c For this local-influence application, you will want beta to be
c   pretty small.
      beta = 0.05
...
c Use the barnes oi scheme to grid the station data.
      ...
      print *,'Working on beta =',beta,'  and original dn=',dn_orig
...
c Write the output data to a grads file.
      write(41,rec=1) ((grid(i,j),i=1,nx),j=1,ny)
