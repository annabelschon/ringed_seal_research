...
c   file for use in SnowModel.
      ...
      integer nx,ny,ntypes,i,j,jj,i_start,idat,k

      real deltax,deltay,xmn,ymn,undef

      real, allocatable :: topo(:,:)
      real, allocatable :: veg1(:,:)
      real, allocatable :: veg2(:,:)

      parameter (ntypes=25)
      integer num_in_class(ntypes)
      integer i_length,trailing_blanks

      character*32 fname_veg
...
c   search and change the two occurances of '32'.  32 is the
c   length of "fname_veg".
      data fname_veg /'NA_NALCMS_landcover_2020_30m.tif'/
...
c Define the array dimensions now that you know nx and ny.
...
c Read in enough information to create a _km .ctl file.
  ...
  106 format (9x,f12.2)
      ...
      call mk_topo_vege_km_ctl (nx,ny,deltax,deltay,xmn,ymn)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine mk_topo_vege_km_ctl (nx,ny,deltax,deltay,xmn,ymn)
