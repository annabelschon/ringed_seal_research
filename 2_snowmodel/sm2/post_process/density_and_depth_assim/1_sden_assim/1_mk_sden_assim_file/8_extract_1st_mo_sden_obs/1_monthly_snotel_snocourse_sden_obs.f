c 1_monthly_snotel_snowcourse_sden_obs.f

c This program is looking at the available SNOTEL and SNOW
c   COURSE sden obs, and extracting the observation locations
c   and the dates you want to use in your assimilation.

c Remember that the SnowModel SWE data assimilation code assumes
c   that any assimilation in a given similation year must have
c   the same stations for all assimilation times.  In other words,
c   you can't have 4 stations on 1 Feb and 3 stations on 1 Apr;
c   in addition, your 3 stations on 1 Apr must be the same 3
c   stations that you had on 1 Feb.  But, for example, in a
c   different year, you can have 4 other stations on 1 Jan and
c   those same 4 stations on 1 Mar.

c The general assumption in this code is that all years in the
c   assimilation will assimilate the same number of months, and
c   the same months, for all the years.  And the same number and
c   same stations in each year, but the number of stations and
c   which stations are used can vary from year to year.

c You pick the month(s) you want to assimilate, and the code
c   looks for valid station data in those months.  If you are
c   just assimilating data from one month (like 1 Apr), then all
c   stations with valid data in that month and for that year
c   will be used.  If you are using more than one month, then
c   it will toss stations that do not have valid data for all
c   of those months.

c This code assumes the days and months you are assimilating are
c   the same for all years.  But it could be modified to relax
c   this assumption; SnowModel does not require that.

      implicit none

      integer nyears,nmonths,ndays,nstns,nobsdates,nyears_max,
     &  nstns_max,ndays_max,n

      parameter (nyears_max=50)
      parameter (ndays_max=20000)
      parameter (nstns_max=4000)

c This is the number of months in a year.
      parameter (nmonths=12)

      parameter (nobsdates=nyears_max*nmonths)

      real sden(nstns_max,ndays_max)
      real sden_all_obs(nstns_max,ndays_max)
      real sden_used_obs(nstns_max,ndays_max)

      integer nnnyr(nyears_max,nmonths),nnnmo(nyears_max,nmonths),
     &  nnndy(nyears_max,nmonths)

      real sden_tmp(nstns_max,nyears_max,nmonths)

      character*40 form

      real undef

      integer nndy,iyr_start,imo_start,k,nday,kk,idy_start,nmonts,
     &  nnmo,nmo,nnyr,nyr,irec,ioptn,julian,julian_start,icount,
     &  nmonth,nyear,iundef,month_first,month_last,nmo_diff

      integer isden_index(nstns_max,nyears_max,nmonths)
      integer irec_save(nyears_max,nmonths)
...
c Define the first and last months you want to include in the
c   assimilation.  Here Sep is 1, Jan is 5, Apr is 8, etc.  If
c   you just want to assimilate one month, then set month_first
c   == month_last.
c     month_first = 7
c     month_last = 7

c Note, these have to be the same number, because the code is
c   not set up to deal with more than one assimilation time.
      month_first = 8
      month_last = 8

      if (month_first.ne.month_last) then
        print *, 'This implementation only allows 1 obs date'
        stop
      endif

c This is the day of the month you are going to assimilate (like
c   1 Oct).  What I am going to do here is extract all of the
c   sden obs that are available on nndy of the month.
      nndy = 1
...
c Read in the number of days in the SnowModel run, and the
c   simulation start date.
      open (72,file=path//'3_extract_time/snotel_maxiter_ioffset.dat')

      read (72,99) iyr_start
      read (72,99) imo_start
      read (72,99) idy_start
      read (72,*)
      read (72,*)
      read (72,*)
      read (72,*)
...
c Open the SNOTEL station data for this station.  The reason I am
c   using the .gdat file version is because it has already dealt
c   with the "days" that are missing from the raw SNOTEL records.
c   Using this also means that you have to use the calendar
c   program to find out where you are in time.
      open (21,file=
     &  '../7_linear_interp_sden_obs/snotel_snocrs_linear_interp.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns)

c Read in the sden obs.
      do nday=1,ndays
        read (21,rec=nday) (sden(k,nday),k=1,nstns)
      enddo

c For the purposes of creating a sden_obs.dat assimilation file,
c   set any sden=0.0 values to undef, because I don't want to
c   assimilate 0.0 values (because a precipitation correction
c   (multiplication) factor is meaningless in this situation).
      do nday=1,ndays
        do k=1,nstns
          if (sden(k,nday).eq.0.0) sden(k,nday) = undef
        enddo
      enddo

c Loop through each of the SNOTEL and SNOW COURSE stations in
c   the SnowModel domain.
      do k=1,nstns

c Initialize a sden array to save just the "nndy-Month" observations.
        do nday=1,ndays
          sden_all_obs(k,nday) = undef
          sden_used_obs(k,nday) = undef
        enddo

        do nyr=1,nyears
          do nmo=1,nmonths
            sden_tmp(k,nyr,nmo) = undef
          enddo
        enddo
...
c Find the records that correspond to the first of the months of
c   interest.
        do nyr=1,nyears
c         print *
          do nmo=1,nmonths

c This is the time index associated with this date.
            kk = (nyr - 1) * nmonths + nmo
c           print *, kk

c This gives you the right calendar months and years from months
c   that go from 1 to 12.
            if (nmo.le.(12-imo_start+1)) then
              nnmo = nmo + imo_start - 1
              nnyr = nyr + iyr_start - 1
            else
              nnmo = nmo - (12-imo_start+1)
              nnyr = nyr + iyr_start
            endif

c Find the irec of this obs date.
            call calndr (ioptn,nndy,nnmo,nnyr,julian)
            irec = julian - julian_start + 1

c           print *, nyr,nmo,nnyr,nnmo,nndy,irec,sden(k,irec)
c           print *, nyr,nmo,irec,ndays

            nnnyr(nyr,nmo) = nnyr
            nnnmo(nyr,nmo) = nnmo
            nnndy(nyr,nmo) = nndy

c This is done to deal with the case of an incomplete final
c   simulation year.
            if (irec.le.ndays) then
              sden_tmp(k,nyr,nmo) = sden(k,irec)
              sden_all_obs(k,irec) = sden(k,irec)
            else
              sden_tmp(k,nyr,nmo) = undef
              sden_all_obs(k,irec) = undef
            endif

            irec_save(nyr,nmo) = irec
...
c Create a simple index that has 0 for undef sden, and 1 for
c   valid sden values.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (sden_tmp(k,nyear,nmonth).ne.undef) then
              isden_index(k,nyear,nmonth) = 1
            else
              isden_index(k,nyear,nmonth) = 0
            endif
          enddo
        enddo
      enddo

c Print this information to the screen.
      do nyear=1,nyears
        print *
        do k=1,nstns
          write (*,101) nyear,k,(isden_index(k,nyear,n),n=1,nmonths)
        enddo
      enddo

c Modify this index to identify which months we want to use and
c   which we do not.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (nmonth.lt.month_first) isden_index(k,nyear,nmonth) = 0
            if (nmonth.gt.month_last) isden_index(k,nyear,nmonth) = 0
          enddo
        enddo
      enddo
...
c Count to see whether there are valid values in each month over
c   the selected months.  If there are not, mask them out.  Also
c   deal with a possible incomplete final year.
      if (nmonts.eq.0) then

        nmo_diff = month_last - month_first + 1
        do nyear=1,nyears
          do k=1,nstns
            icount = 0
            do nmonth=1,nmonths
              if (isden_index(k,nyear,nmonth).eq.1) icount = icount + 1
            enddo
            if (icount.ne.nmo_diff) then
              do nmonth=1,nmonths
                isden_index(k,nyear,nmonth) = 0
              enddo
            endif
...
c Process the full and partial years.
        nmo_diff = month_last - month_first + 1
        do nyear=1,nyears-1
          do k=1,nstns
            icount = 0
...
c Use this array info to remove unwanted data from the sden_tmp
c   array.
      do nyear=1,nyears
        do nmonth=1,nmonths
          do k=1,nstns
            if (isden_index(k,nyear,nmonth).eq.0) then
              sden_tmp(k,nyear,nmonth) = undef
            endif
          enddo
        enddo
      enddo

c Create a grads-writable array that just contains the sden data
c   that will be used in the assimilation.
      do nyear=1,nyears
        do nmonth=1,nmonths
          nday = irec_save(nyear,nmonth)
          do k=1,nstns
            if (sden_tmp(k,nyear,nmonth).ne.undef) then
              sden_used_obs(k,nday) = sden_tmp(k,nyear,nmonth)
            endif
          enddo
        enddo
      enddo

c Use the above information to make a file of the results that
c   will be used in the assimilation.
      open (81,file='sden_obs_for_assim.dat')
      ...
      if (nmonts.eq.0) then
        ...
        nmo_diff = month_last - month_first + 1
        do nyear=1,nyears
          write (81,95) nmo_diff
          do nmonth=month_first,month_last
            write (81,form) nnnyr(nyear,nmonth),nnnmo(nyear,nmonth),
     &        nnndy(nyear,nmonth),(sden_tmp(k,nyear,nmonth),k=1,nstns)
          enddo
        enddo
  ...
  91  format ('(3i8,',i5,'f12.3)')

c Also make a grads file of these sden obs.
      open (61,file='snotel_sden_obs_used.gdat',
     &  form='unformatted',access='direct',recl=4*2*nstns,
     &  status='replace')

      do nday=1,ndays
        write (61,rec=nday)
     &    (sden_all_obs(k,nday),k=1,nstns),
     &    (sden_used_obs(k,nday),k=1,nstns)
      enddo

c Make the corresponding .ctl file.
      call make_ctl_file (nstns,ndays,iyr_start)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine make_ctl_file (nstns,ndays,iyr_start)

      implicit none

      integer nstns,iyr_start,ndays,nvars,nvar
...
c   quotes, and they cannot be more than 15 characters long.
      data var_string /
     &  'sdenall  0  0 sden obs (m)',
     &  'sdenused 0  0 sden obs (m)'/

      open (71,file='snotel_sden_obs_used.ctl')
   ...
   51 format ('DSET ^snotel_sden_obs_used.gdat')
