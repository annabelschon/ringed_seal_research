...
c             print *, nday,irec
              if (snod(k,irec).ne.undef) then
                print *, k,nyear,nday,irec,snod(k,irec)
              endif
              snod(k,irec) = undef
            enddo
...
c Save the data.
      open (41,file='snotel_snocrs_snod.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns,
     &  status='replace')

      do nday=1,ndays
        write (41,rec=nday) (snod(k,nday),k=1,nstns)
      enddo
...
c   quotes, and they cannot be more than 15 characters long.
     ...
     &  'snod     0  0 snod (m)'/
      ...
      open (71,file='snotel_snocrs_snod.ctl')
   ...
   51 format ('DSET ^snotel_snocrs_snod.gdat')
