c 1_monthly_snotel_snowcourse_snod_obs.f

c This program is looking at the available SNOTEL and SNOW
c   COURSE snod obs, and extracting the observation locations
...
c This is the number of months in a year.
      ...
      real snod(nstns_max,ndays_max)
      real snod_all_obs(nstns_max,ndays_max)
      real snod_used_obs(nstns_max,ndays_max)

      integer nnnyr(nyears_max,nmonths),nnnmo(nyears_max,nmonths),
     &  nnndy(nyears_max,nmonths)

      real snod_tmp(nstns_max,nyears_max,nmonths)
     ...
     &  nmonth,nyear,iundef,month_first,month_last,nmo_diff
      ...
      integer isnod_index(nstns_max,nyears_max,nmonths)
...
c   snod obs that are available on nndy of the month.
      nndy = 1
...
c   program to find out where you are in time.
      open (21,file=
     &  '../7_linear_interp_snod_obs/snotel_snocrs_linear_interp.gdat',
     &  form='unformatted',access='direct',recl=4*1*nstns)

c Read in the snod obs.
      do nday=1,ndays
        read (21,rec=nday) (snod(k,nday),k=1,nstns)
      enddo

c For the purposes of creating a snod_obs.dat assimilation file,
c   set any snod=0.0 values to undef, because I don't want to
c   assimilate 0.0 values (because a precipitation correction
c   (multiplication) factor is meaningless in this situation).
      do nday=1,ndays
        do k=1,nstns
          if (snod(k,nday).eq.0.0) snod(k,nday) = undef
        enddo
      enddo
...
c Initialize a snod array to save just the "nndy-Month" observations.
        do nday=1,ndays
          snod_all_obs(k,nday) = undef
          snod_used_obs(k,nday) = undef
        enddo

        do nyr=1,nyears
          do nmo=1,nmonths
            snod_tmp(k,nyr,nmo) = undef
          enddo
        enddo
...
c           print *, nyr,nmo,nnyr,nnmo,nndy,irec,snod(k,irec)
...
c   simulation year.
            if (irec.le.ndays) then
              snod_tmp(k,nyr,nmo) = snod(k,irec)
              snod_all_obs(k,irec) = snod(k,irec)
            else
              snod_tmp(k,nyr,nmo) = undef
              snod_all_obs(k,irec) = undef
            endif
...
c Create a simple index that has 0 for undef snod, and 1 for
c   valid snod values.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (snod_tmp(k,nyear,nmonth).ne.undef) then
              isnod_index(k,nyear,nmonth) = 1
            else
              isnod_index(k,nyear,nmonth) = 0
            endif
          enddo
        enddo
      enddo
...
c Print this information to the screen.
      do nyear=1,nyears
        print *
        do k=1,nstns
          write (*,101) nyear,k,(isnod_index(k,nyear,n),n=1,nmonths)
        enddo
      enddo
...
c   which we do not.
      do nyear=1,nyears
        do k=1,nstns
          do nmonth=1,nmonths
            if (nmonth.lt.month_first) isnod_index(k,nyear,nmonth) = 0
            if (nmonth.gt.month_last) isnod_index(k,nyear,nmonth) = 0
          enddo
        enddo
      enddo
      ...
      if (nmonts.eq.0) then
        ...
        do nyear=1,nyears
          do k=1,nstns
            icount = 0
            do nmonth=1,nmonths
              if (isnod_index(k,nyear,nmonth).eq.1) icount = icount + 1
            enddo
...
c Use this array info to remove unwanted data from the snod_tmp
c   array.
      do nyear=1,nyears
        do nmonth=1,nmonths
          do k=1,nstns
            if (isnod_index(k,nyear,nmonth).eq.0) then
              snod_tmp(k,nyear,nmonth) = undef
            endif
          enddo
        enddo
      enddo

c Create a grads-writable array that just contains the snod data
c   that will be used in the assimilation.
      do nyear=1,nyears
        do nmonth=1,nmonths
          ...
          do k=1,nstns
            if (snod_tmp(k,nyear,nmonth).ne.undef) then
              snod_used_obs(k,nday) = snod_tmp(k,nyear,nmonth)
            endif
          enddo
        enddo
      enddo
...
c   will be used in the assimilation.
      open (81,file='snod_obs_for_assim.dat')
     ...
     &        nnndy(nyear,nmonth),(snod_tmp(k,nyear,nmonth),k=1,nstns)
          enddo
        enddo
...
c Also make a grads file of these snod obs.
      open (61,file='snotel_snod_obs_used.gdat',
     &  form='unformatted',access='direct',recl=4*2*nstns,
     &  status='replace')

      do nday=1,ndays
        write (61,rec=nday)
     &    (snod_all_obs(k,nday),k=1,nstns),
     &    (snod_used_obs(k,nday),k=1,nstns)
      enddo
...
c   quotes, and they cannot be more than 15 characters long.
     ...
     &  'snodused 0  0 snod obs (m)'/
      ...
      open (71,file='snotel_snod_obs_used.ctl')
   ...
   51 format ('DSET ^snotel_snod_obs_used.gdat')
