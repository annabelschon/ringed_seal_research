c 1_mk_mod_obs_snod_data.f

c Here I am making strings of snod data that has all of the obs
...
c Open the SnowModel input file.
      open (21,file=
     &  path(1:i_len)//'snod_assim.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny)

c Open the snod observation data file.
      open (31,file=
     &  '../1_mk_snod_assim_file/9_mk_snod_assim_file/'//
     &  'snod_obs.dat')

c Output files.
      open (41,file='snod_obs_mod.dat')

      open (51,file='snod_obs_mod.gdat',
...
c   time.
        read (21,rec=iobs_rec) ((snod(i,j),i=1,nx),j=1,ny)

        read (31,*) n_den_pts
        do k=1,n_den_pts
          icount = icount + 1
          read (31,*) id(k),xstn(k),ystn(k),snod_obs(icount)
...
c Extract the data at the obs grid cell.
          snod_mod(icount) = snod(ii(k),jj(k))

c Save the obs and mod data.
          write (41,91)
     &      k,iiyr,iimo,iidy,snod_obs(icount),snod_mod(icount)
...
c   from this step.
        if (snod_mod(k).lt.0.005) snod_mod(k) = undef

        write (51,rec=k) snod_obs(k),snod_mod(k)
      enddo
