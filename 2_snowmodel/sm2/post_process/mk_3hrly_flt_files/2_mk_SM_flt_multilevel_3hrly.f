c 2_mk_SM_flt_multilevel_3hrly.f

c ****
c This program is specifically dealing with SnowModel multi-
c   layer .gdat files, like: multilayer_temp.gdat and the
c   corresponding multilayer_temp.ctl file.  This means that
c   you have chosen option print_multilayer = 2.0 in the
c   snowmodel.par file.
c ****
...
c Define the SnowModel simulation domain dimensions.
      parameter (nx=33,ny=32)

c This is "max_layers + 1" from the .par file (or nz_max in the
c   /code/snowmodel.inc file).
      parameter (nz=19)

c This just needs to be big enough to deal with the number of
c   time steps in the simulation.  100 years should be enough.
      parameter (maxiter=100*365*8) 

c This code just processes one variable.  You can name it here.
c   It assumes that you have chosen option print_multilayer =
c   2.0 in the snowmodel.par file.
      character*4 varname

      data varname /'temp'/

      real var(nx,ny,nz)
      real var_layer(nx,ny)
      ...
      integer iyr_start,imo_start,idy_start,iyr_end,imo_end,
     &  idy_end,ioptn,julian_end,julian_start,julian,kk,iday,
     &  imonth,iyear,ihrs_in_day,last,nwrites,i,j,k,nlayer
...
c Define the run name that will go into the float file names.  This
c   is designed to have the format: "NAME_SM_reso".
      character*12 run_name
      parameter (run_name='MULTI_LAYERS')
...
c Provide some model configuration details.
      xmn = 736500.00D0
      ymn = 4307500.00D0
      deltaxy = 1000.0
...
c Define the start and end of the data block.  The code that
c   follows will handle the 3 hour time steps.
      iyr_start = 2019
      imo_start = 10
      idy_start = 15
...
c Find the Julian day at the start of the data period.
      ioptn = 3
      call calndr (ioptn,idy_start,imo_start,iyr_start,julian_start)

c Find the Julian day at the end of the data period.
      call calndr (ioptn,idy_end,imo_end,iyr_end,julian_end)

      nwrites = 8 * (julian_end - julian_start + 1)

      print *
      print *,'# data writes in the flt conversion = ',nwrites
      print *

c Build the date-time arrays that correspond to each model time step.
      ihrs_in_day = 24
...
c Input file.
      open (21,file=path_in//'multilayer_'//varname//'.gdat',
     &  form='unformatted',access='direct',recl=4*nx*ny*nz)

c Extract and write out the data as .flt files.
      print *
      do k=1,nwrites

        if (idy(k).eq.1 .and. ihr(k).eq.3) then
          print *, varname,k,iyr(k),imo(k),idy(k),ihr(k)
        endif

        read (21,rec=k) (((var(i,j,nlayer),i=1,nx),j=1,ny),nlayer=1,nz)

c Loop through the layers.
        do nlayer=1,nz

          write(clayer,'(i2.2)') nlayer

c Extract the layer of interest from this time slice.
          do j=1,ny
            do i=1,nx
              var_layer(i,j) = var(i,j,nlayer)
            enddo
          enddo

c Save the data as .flt files.
          call flt_outputs(nx,ny,iyr(k),imo(k),idy(k),ihr(k),
     &      path_out,var_layer,varname,xmn,ymn,deltaxy,undef,
     &      run_name,clayer)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine flt_outputs(nx,ny,iyear,imonth,iday,ihour,
     &  path_out,var,cvar,xmn,ymn,deltaxy,undef,
     &  run_name,clayer)
      ...
      character*2 dd
      character*2 hh
      character*13 date_string
      character*38 f_name1
      character*38 f_name2
...
c Create the date string.
      ...
      write(dd,'(i2.2)') iday
      write(hh,'(i2.2)') ihour
      date_string = yyyy//'_'//mm//'_'//dd//'_'//hh

c This is the file name without the path.
      f_name1 =
     &  run_name//'_'//cvar//'_'//date_string//'_'//clayer//'.flt'
      f_name2 =
     &  run_name//'_'//cvar//'_'//date_string//'_'//clayer//'.hdr'
