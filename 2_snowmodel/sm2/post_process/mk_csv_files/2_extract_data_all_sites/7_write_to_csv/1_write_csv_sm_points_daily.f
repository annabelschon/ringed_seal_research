c 1_write_csv_sm_points_daily.f

c Read SnowModel .gdat file outputs, and output time series at
c   specific grid cells, for different variables.

c Note: For this program, the points have already been extracted
c   and written out in a previous step.

      implicit none

      integer npts,nt,nvars1,nvars2
      integer iyr_start,imo_start,idy_start,iyr_end,imo_end,idy_end
      integer ioptn,julian_end,julian_start,julian
...
c Define the header line.
      character header*(*) 
      parameter (header=
     &  'day_number,site_id,year,month,day,'//
     &  'longitude(decimal_degrees),latitude(decimal_degrees),'//
     &  'elevation(masl),'//
     &  'land-cover_class(1-30),'//

     &  'air_temperature(deg_C),'//
     &  'relative_humidity(%),'//
     &  'snow_depth(m),'//
     &  'snow_density(kg/m^3),'//
     &  'snow-water-eqivalent_depth(m)')

      undef = -9999.0

c Open the .gdat input files.
      open (unit=81,file='../5_extract/point_variables.gdat',
     &  form='unformatted',access='direct',recl=4*nvars1*npts)

      open (unit=82,file='../5_extract/point_topovege.gdat',
     &  form='unformatted',access='direct',recl=4*nvars2*npts)

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c Read the grid-point lat-lon coordinates.
      open (unit=51,file='../2_ll_to_proj/stn_ll.dat')

      do k=1,npts
        read (51,*) glon(k),glat(k)
      enddo

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c This is set up to work for daily time steps.
      iyr_start = 2010
      imo_start = 9
      idy_start = 1
...
c Find the Julian day at the end of the data stream.
      ...
      print *,'# days in simulation = ',julian_end - julian_start + 1

c Read in the topo and vege information.
      read (82,rec=1)
     &  (topo_gdat(k),k=1,npts),
     &  (vege_gdat(k),k=1,npts)

c Create a file name for each file, and open the files.
      do k=1,npts
...
c Open the output files.
     ...
     &    'data_daily/SM_Data_daily_'//stn_name//stnid//'.csv')
...
c Loop through the days in the simulation, reading the data and
c   saving the information to the output files.
      do iter=1,nt

c Find date
        ioptn =  4
        julian = julian_start + iter - 1
...
c If you are not writing out the snow data to its full resolution
c   (e.g., snow depth to 4 decimal places, instead of 6 or 7),
c   you can have a situation where there is zero snow depth, with
c   a non-undefined snow density.  Correct that here.
c       do k=1,npts
c         if (nint(10000.0*snod_gdat(k)).eq.0) then
c           sden_gdat(k) = undef
c           swed_gdat(k) = 0.0
c         endif
c       enddo

c Write each point's data to a separate .csv file.
        do k=1,npts

          write (200+k,998)
     &      iter,k,iyear,imonth,iday,
...
c   to 5 decimal places, the topo data to be written out to 1
c   decimal place, and the moisture variables to be written
c   out to 6 decimal places (=0.001 mm), and the temperature and
c   humidity to 2 decimal places, and the snow density to 3
c   decimal places.
c 998 format (5(I0,","),2(F0.5,","),F0.1,",",*(G0.6,:,","))

  998 format (5(I0,","),2(F0.5,","),(F0.1,","),(I0,","),2(F0.2,","),
     &  (F0.6,","),(F0.3,","),(F0.6))
