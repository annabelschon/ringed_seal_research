c 1_extract_gps_points_4variables.f

c In order to extract single-value data from the (x,y,t) .gdat
c   data cube, you need to be able to define the location in
c   that cube using the following equation (for daily data;
c   there is a different equation for 3-hourly data, see the
c   code below):

c   irec = (time_rec - 1) * nx * ny + (j - 1) * nx + i

c This number (irec) can be huge.  The max size of a single
c   precision integer is (2**31-1) = 2,147,483,647 (about
c   2.2 GB).  This is big enough to deal with a nx=1000,
c   ny=1000, daily outputs, for 5.9 years.  Anything bigger
c   requires double precision for irec (and rec) when accessing
c   single values out of the (x,y,t) single-variable data cube.

c The solution is to tell the compiler to use double precision
c   integers for all calculations.  integer*8 values can be as
c   large as: 9,223,372,036,854,775,807.  This is plenty big
c   enough for SnowModel operations.

c This is accomplished by implementing the "-fdefault-integer-8"
c   compiler option; this sets all of the integers and integer
c   operations to integer*8.  This means that irec can be as
c   large as: 9,223,372,036,854,775,807.  This is what is
c   allowing us to access single-value data in the direct access
c   data cube, when the data cube is over about 8.8 GB (or
c   contains over ~2.2 GB worth of numbers.  Note that it is not
c   enough to just set irec = integer*8, because "rec" also
c   needs to be set the same.  And the only way to do that is
c   with the compiler option.

c gfortran -fdefault-integer-8 extract_gps_points_4variables_use.f

c Alternatively, if you can't compile with "-fdefault-integer-8"
c   Glen has other codes that can do what needs to be done; they
c   just are not as efficient as this version of the code.

      implicit none

      integer nx,ny,nt,nobs

      parameter (nx=1162,ny=854)
      parameter (nt=14610)
      parameter (nobs=152000)

      real x,y,undef,dec_hr

      real tair,snod,sden,wspd

      integer julian,k,idy,imo,iyr,irec,nrecs,iyr_start,imo_start,
     &  idy_start,iyr_end,imo_end,ii,jj,idy_end,ioptn,julian_end,
     &  julian_start,id,irecs_in_day,itime_rec_days,itime_rec_3hrs,
     &  ihr,imn

      double precision xmn,ymn,deltax,deltay

      character*5 run

c SnowModel data location.
      character path*(*)
      parameter (path='/data4/above/sm_2/outputs/wo_assim/')

      undef = -9999.0

c Define whether this is going to extract data from daily or
c   3-hourly SnowModel outputs.  This is done by setting, run =
c   '3hrly' or run = 'daily'.
      run = '3hrly'
c     run = 'daily'

c Open the GPS data input file.
      if (run.eq.'3hrly') then

        open (31,file='GPS_id_yr_mo_dy_hr_mn_x_y.dat')

      elseif (run.eq.'daily') then

        open (31,file='GPS_id_yr_mo_dy_x_y.dat')

      endif

c This indicates I will be extracting single values from the
c   data cube.
      nrecs = 1

c SnowModel files.
      open (21,file=path//'tair.gdat',
     &  form='unformatted',access='direct',recl=4*nrecs)
      open (22,file=path//'snod.gdat',
     &  form='unformatted',access='direct',recl=4*nrecs)
      open (23,file=path//'sden.gdat',
     &  form='unformatted',access='direct',recl=4*nrecs)
      open (24,file=path//'wspd.gdat',
     &  form='unformatted',access='direct',recl=4*nrecs)

c Output file.
      open (61,file='extract_data_test.dat')

cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

c Provide some run-dependent information.
      deltax = 3000.0D0
      deltay = 3000.0D0
...
c Calculate the Julian start and end dates of this simulation.

c This is the begining of the period of interest.
      iyr_start = 1980
      imo_start = 9
      idy_start = 1

c This is the end of the period of interest.
      iyr_end = 2020
      imo_end = 8
      idy_end = 31
...
c Process the date and coordinate information to find the
c   corresponding data record in the data-block.
      do k=1,nobs

c Read in the data and coordinate information.  If this is
c   going to read from a daily .gdat file, then this is
c   simple, and the input file will take the form:
c     id,iyr,imo,idy,x,y

c   But if this is going to read from a 3-hourly .gdat file,
c   your corresponding GPS input file will have additional
c   columns of hour and minutes:
c     id,iyr,imo,idy,ihr,imn,x,y

c   In addition, this input file format is a little more
c   specific regarding how the hour and minutes are defined;
c   these, and the relationship to the day number, must follow
c   SnowModel conventions.  In addition, you can't have some
c   things in the input file date string that don't make sense,
c   like, ihr = 8 and imn = 60; this should be ihr = 9 and imn
c   = 0.  It also requires (assumes) the standard SnowModel hour
c   0 stuff [hour 0 is for the previous day (date)].
        if (run.eq.'3hrly') read (31,*) id,iyr,imo,idy,ihr,imn,x,y
        if (run.eq.'daily') read (31,*) id,iyr,imo,idy,x,y

        ioptn = 3
        call calndr (ioptn,idy,imo,iyr,julian)

        if (julian.lt.julian_start  .or.  julian.gt.julian_end) then
          print *, 'observation is outside the time domain'
          tair = undef
          snod = undef
          sden = undef
          wspd = undef
        ...
        itime_rec_days = julian - julian_start + 1

        if (run.eq.'3hrly') then

c Convert the hour and minutes to decimal hours.
          dec_hr = real(ihr) + real(imn) / 60.0

c Convert the decimal hour to a 3-hourly time record (1-8).
c   Note that this also has to deal with the special case of
c   hour = 0.0 (i.e., 24.0) is the day before, not the new
c   day.
c     Here I want:
c       0.0 < dec_hr <= 3.0, = 1
c       3.0 < dec_hr <= 6.0, = 2
c       ...
c       18.0 < dec_hr <= 21.0, = 7
c       21.0 < dec_hr <= 0.0, = 8

c The function, CEILING, always rounds up.  It requires a
c   Fortran 95 compiler, like gfortran.  The 0.0 check takes
c   care of the special hour issue.  You need to make sure the
c   "day and hour" notation is also correct in your input
c   dataset (day = 2 and hour = 0 is used instead of day = 1
c   and hour = 24).
          if (dec_hr.eq.0.0) then
            itime_rec_3hrs = 8
            itime_rec_days = itime_rec_days - 1
          else
            itime_rec_3hrs = int(CEILING(dec_hr/3.0))
          endif
...
c Process the coordinates.  ii and jj are the coordinates within
c   the 2D domain.
        ii = 1 + nint((x - xmn) / deltax)
        jj = 1 + nint((y - ymn) / deltay)

c       print *, ii,jj

        if (ii.lt.1  .or.  ii.gt.nx  .or.  jj.lt.1  .or.  jj.gt.ny) then
          print *, 'observation is outside the spatial domain'
          tair = undef
          snod = undef
          sden = undef
          wspd = undef
...
c Find the data-block record.
        irec =
     &    ((itime_rec_3hrs - 1) + irecs_in_day * (itime_rec_days - 1)) *
     &    (nx * ny) + (jj - 1) * nx + ii
c       print *, id,irec
c       print *,
c    &    id,iyr,imo,idy,ihr,imn,irec,itime_rec_3hrs,itime_rec_days

c Extract that data record from the data-block.
        read (21,rec=irec) tair
        read (22,rec=irec) snod
        read (23,rec=irec) sden
        read (24,rec=irec) wspd

c Convert the snow depth to cm.
        if (snod.ne.undef) snod = 100.0 * snod

   99 continue

c Save the data.
        write (61,92) k,iyr,imo,idy,dec_hr,ii,jj,tair,snod,sden,wspd
c       write (61,91) k,iyr,imo,idy,dec_hr,ii,jj,tair,snod,sden,wspd

      enddo

c This fancy formatting required gfortran to work.
...
c 91  format (*(G0.6,:,","))
  ...
  92  format (i10,3i7,f7.2,2i7,4f10.2)
