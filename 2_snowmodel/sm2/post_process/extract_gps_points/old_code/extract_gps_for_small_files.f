c extract_variable.f
      ...
      parameter (nx=290,ny=430)
      parameter (nt=2557)
      parameter (nobs=152000)

      real tair,snod,sden,wspd
      real t,tyr,tmo,tdy,x,y

      integer julian,k,i,j,idy,imo,iyr,itime_rec,irec,
     &  iyr_start,imo_start,idy_start,iyr_end,imo_end,
     &  idy_end,ioptn,julian_end,julian_start

      double precision xmn,ymn,deltax,deltay

c SnowModel data location.
      character path*(*)
      parameter (path='/data3/svalbard/sm_brogger_1/outputs/wo_assim/')
...
c GPS data file.
      open (31,file='input/gps_x_y_t.gdat',
     &  form='unformatted',access='direct',recl=4*6)
...
c Read in the data and coordinate information.
        read (31,rec=k) x,y,t,tyr,tmo,tdy
...
c Process the coordinates.  i and j are the coordinates within
c   the 2D domain.
        i = 1 + nint((x - xmn) / deltax)
        j = 1 + nint((y - ymn) / deltay)

c Find the data-block record.
        irec = (itime_rec - 1) * nx * ny + (j - 1) * nx + i
...
c Save the data.  Convert the snow depth to cm.
        write (61,88) k,iyr,imo,idy,i,j,tair,100.0*snod,sden,wspd

      enddo

   88 format (i10,5i7,4f10.2)
