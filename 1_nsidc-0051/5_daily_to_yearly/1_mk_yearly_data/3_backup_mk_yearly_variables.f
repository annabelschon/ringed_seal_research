...
c   to a .gdat file and it will create an associated .ctl file.
     ...
     &  imo_start,idy_start,irec_start
      ...
      character path*(*)
      parameter (path =
c    &  '/data1/working/snowmodel/sm/outputs/wo_assim/')
     &  '../../../outputs/wo_assim/')
...
c   'snowcover' subroutine).
     ...
     o    spre_sum)
        ...
        call find_max_date (nx,ny,path,irec_start,ndays,undef,
     &    doy_start_of_sim,doy_31Dec,
     i    'snod.gdat',
     o    snod_max,snod_max_dos,snod_max_doy)

        call find_max (nx,ny,path,irec_start,ndays,undef,
     i    'swed.gdat',
     o    swed_max)

        call calc_ave (nx,ny,path,irec_start,ndays,undef,
     i    'tair.gdat',
     o    tair_ave)
     ...
     o    total_snow_days)
        ...
        call find_median (nx,ny,path,irec_start,ndays,undef,
     i    'snod.gdat',
     i    snow_first_dos,snow_last_dos,
     o    snod_median)

        call rain_on_snow (nx,ny,path,irec_start,ndays,undef,
     i    'rpre.gdat','swed.gdat',
     o    ros_days)

c Save the data.
        write (61,rec=nyear)
     &    ((prec_sum(i,j),i=1,nx),j=1,ny),
     &    ((rpre_sum(i,j),i=1,nx),j=1,ny),
     &    ((spre_sum(i,j),i=1,nx),j=1,ny),

     &    ((snod_max(i,j),i=1,nx),j=1,ny),
     &    ((snod_max_dos(i,j),i=1,nx),j=1,ny),
     &    ((snod_max_doy(i,j),i=1,nx),j=1,ny),
     &    ((swed_max(i,j),i=1,nx),j=1,ny),

     &    ((tair_ave(i,j),i=1,nx),j=1,ny),
     ...
     &    ((snod_median(i,j),i=1,nx),j=1,ny),

     &    ((ros_days(i,j),i=1,nx),j=1,ny)
