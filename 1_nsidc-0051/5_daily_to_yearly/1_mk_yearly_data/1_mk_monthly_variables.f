...
c   to a .gdat file and it will create an associated .ctl file.
     ...
     &  imo_start,idy_start,irec_start,k

      real doy_start_of_sim,doy_31Dec,undef

      parameter (nx=304,ny=448)

      parameter (nvars=1)

      real monthly_avg(nx,ny,12)
c     real ice_above_thres(nx,ny)
...
c Output file.
      open (61,file=path_out//'annabel_monthly.gdat',
     &  form='unformatted',access='direct',recl=4*nvars*nx*ny*12,
     &  status='replace')
...
c   'snowcover' subroutine).
        ...
        call calc_monthly_avg(nx,ny,path,irec_start,ndays,undef,
     i    'ice_conc_extracted_domain.gdat',iyr_start,
     o    monthly_avg,doy_31Dec,doy_start_of_sim,nyear)

c Save the data.

        write (61,rec=nyear)
     &    (((monthly_avg(i,j,k),i=1,nx),j=1,ny),k=8,12),
     &    (((monthly_avg(i,j,k),i=1,nx),j=1,ny),k=1,7)
...
c   year will be 2005).
c     call make_yearly_ctl_file (nx,ny,nyears,nvars,iyr_start,
c    &  var_string)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine calc_monthly_avg(nx,ny,path,irec_start,ndays,undef,
     & fname,iyr_start,monthly_avg,doy_31Dec,doy_start_of_sim,nyear)

C     Calculate monthly average for sea ice concentration at each grid
C     cell. Use calendr() subroutine option 2 to get corresponding 
C     month for each year.
      ...
      integer nx,ny,i,j,nday,ndays,irec,irec_start,
     &  iyr_start
      integer ioptn,month,nyear,idoy,iyr_now,nday_mo
      
      real doy_31Dec,doy_start_of_sim
      real undef
c     character*(*) path, fname
      real monthly_avg(nx, ny, 12) 
c     Store monthy averages
      real var(nx,ny)
      real monthly_sum(nx,ny, 12)
      integer monthly_count(nx, ny, 12)

c     Use ioptn=2 to, given the day number of the year (idayct) and the
c     year (iyear), compute the day of the month (iday) and the month.
...
c     Initialize arrays,
      ...
      print *
      print *, iyr_start,doy_31Dec,doy_start_of_sim,nyear
      print *

c     Loop over each grid cell in the dataset
      do nday=1,ndays 

c Find the year you are in for each day in the simulation.  You
c   also need to find the doy instead of the model simulation
c   day of simulation year.
        if (nday.gt.nint(doy_31Dec-doy_start_of_sim+1)) then
          iyr_now = iyr_start + nyear
          idoy = nday - nint(doy_31Dec-doy_start_of_sim) - 1
        else
          iyr_now = iyr_start + nyear - 1
          idoy = nday + nint(doy_start_of_sim) - 1
        endif

c       print *, nday,iyr_now,idoy

c     Call calndr to get the month based on day_of_year
        call calndr(ioptn, nday_mo, month, iyr_now, idoy)

        irec = irec_start + nday - 1

        print *, nday,nday_mo,iyr_now,idoy,month,irec

c     Read data for each grid cell
        read(21,rec=irec) ((var(i, j), i=1, nx), j=1, ny)

        do j=1,ny
          do i=1,nx
            if (var(i, j) /= undef) then
              monthly_sum(i, j, month) = monthly_sum(i, j, month) +  
     &                                    var(i,j)
              monthly_count(i, j, month) = monthly_count(i, j, month)+1
            endif
          enddo
        enddo
...
c     Calculate the monthly averages from count and sum
      do month=1, 12 
        do j=1,ny 
          do i=1,nx 
            if (monthly_count(i, j, month) > 0) then
              monthly_avg(i, j, month) = monthly_sum(i, j, month) /  
     &                                    monthly_count(i, j, month)
            else
              monthly_avg(i,j,month)=undef
            endif
          enddo
        enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine rain_on_snow (nx,ny,path,irec_start,ndays,undef,
     i  fname1,fname2,
     o  ros_days)

      implicit none

      integer nx,ny,i,j,ndays,nday,irec_start,irec
      ...
      character*(*) fname1
      character*(*) fname2
      character*120 path_fname1
      character*120 path_fname2

c       read (24,rec=iter) ((rpre_yr(i,j,nday),i=1,nx),j=1,ny)

c Open the input arrays.
      path_fname1 = path//fname1
c     print *, path_fname1
      open (21,file=path_fname1,
     &  form='unformatted',access='direct',recl=4*nx*ny)

      path_fname2 = path//fname2
c     print *, path_fname2
      open (22,file=path_fname2,
     &  form='unformatted',access='direct',recl=4*nx*ny)

c Initialize the ros array.
      do j=1,ny
        do i=1,nx
          ros_days(i,j) = 0.0
        enddo
...
c Loop through the days in this year.
      do nday=1,ndays
        irec = irec_start + nday - 1
c       print *, nday,irec
        read (21,rec=irec) ((rpre(i,j),i=1,nx),j=1,ny)
        read (22,rec=irec) ((swed(i,j),i=1,nx),j=1,ny)

c Count the number of days with rain on snow (3mm rain on 5mm swe).
        do j=1,ny
          do i=1,nx
            if (swed(i,j).ge.0.005 .and. rpre(i,j).ge.0.003) then
              ros_days(i,j) = ros_days(i,j) + 1.0
            endif
          enddo
        enddo
...
c Set oceans to undefined.  Here I have assumed that any undef
c   swed values are ocean grid cells.
      do j=1,ny
        do i=1,nx
          if (swed(i,j).eq.undef) then
            ros_days(i,j) = undef
          endif
        enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine find_median (nx,ny,path,irec_start,ndays,undef,
     i  fname,
     i  snow_first_dos,snow_last_dos,
     o  snod_median)
      ...
      real snod_yr(nx,ny,ndays_max)
      real snod_string(ndays_max)

      real snow_first_dos(nx,ny)
      real snow_last_dos(nx,ny)
      real undef_mask(nx,ny)
      real snod_median(nx,ny)

      real undef

      integer iday,nsnow_days,icount0,icount1,iundef
...
c Extract the string you want to calculate the median of.  Here
c   I have assumed that it will go from snow_first_dos to
c   snow_last_dos, and there will be total_snow_days worth of
c   snod values to process.
      do j=1,ny
        do i=1,nx

c If snow_first_dos(i,j) is undef, then snow_last_dos(i,j) is
c   also undef, so there was never any snow and nsnow_days = 0.
c   If they are both the same, but not undef, then nsnow_days
c   = 1.
          if (snow_first_dos(i,j).eq.undef) then
            nsnow_days = 0
          else
            nsnow_days =
     &        nint(snow_last_dos(i,j) - snow_first_dos(i,j) + 1.0)
          endif

c Count how many grid cells have only 0 or 1 days with snow this
c   year.
          if (nsnow_days.eq.0) icount0 = icount0 + 1
          if (nsnow_days.eq.1) icount1 = icount1 + 1

          if (nsnow_days.ge.1) then
            do nday=1,nsnow_days
              iday = nint(snow_first_dos(i,j)) + nday - 1
              snod_string(nday) = snod_yr(i,j,iday)
            enddo
          endif

c Calculate the median of this snod string.  Send in the string
c   to be sorted, snod_string(total_snow_days), of length,
c   total_snow_days, and return the median value.  Note that
c   the returned string snod_string(total_snow_days), will be
c   returned sorted.  Also note that the median calculation only
c   works if ndays is 2 or greater.
          if (nsnow_days.ge.2) then
            call calc_median (snod_string,nsnow_days,snod_median(i,j))
          elseif (nsnow_days.eq.1) then
            snod_median(i,j) = snod_string(nsnow_days)
          elseif (nsnow_days.eq.0) then
            snod_median(i,j) = 0.0
          else
            print *,'nsnow_days not found'
            stop
          endif
...
c Print out how many grid cells didn't have enough snow days to
c   calculate a median value.
c     print *, 'number of grid cells with 0 days of snow =',icount0
c     print *, 'number of grid cells with 1 day  of snow =',icount1
c     print *
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine calc_median (A,N,xmedian)

c Calculate the median value of a 1-D array using the Heapsort
c   method.

c REFERENCE: NUMERICAL RECIPES, by W.H. Press, B.P. Flannery,
c   S.A. Teukolsky, and W.T. Vetterling, Cambridge University
c   Press, 1986.  Original code written by J-P Moreau, Paris
c   (www.jpmoreau.fr).  This version was modified by Glen Liston
c   to fit SnowModel conventions.

c Given an array A of N numbers, return their median value, xmedian.
c   The array A is modified and returned sorted in ascending order.
...
c Sort an array A of length N in ascending order using the
c   Heapsort method.

c INPUT:
c   N = size of table A
c   A = table to be sorted

c OUTPUT:
c   A = table sorted in ascending order

c NOTE: The Heapsort method is a N Log2 N routine, and can be
c   used for very large arrays.
...
c The index L will be decremented from its initial value during
c   the 'hiring' (heap creation) phase. Once it reaches 1, the
c   index IR will be decremented from its initial value down to
c   1 during the 'retirement-and-promotion' (heap selection)
c   phase.
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     o  total_snow_days)
      ...
      integer icount,iicount,iiicount,idays_old
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine calc_sum (nx,ny,path,irec_start,ndays,undef,
     i  fname,
     o  sum)
...
c Sum the variable over the year.
        do j=1,ny
          do i=1,nx
            sum(i,j) = sum(i,j) + var(i,j)
          enddo
        enddo
...
c   Just look at the last 2D array to do this.
      do j=1,ny
        do i=1,nx
          if (var(i,j).eq.undef) sum(i,j) = undef
        enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine calc_ave (nx,ny,path,irec_start,ndays,undef,
     i  fname,
     o  ave)
...
c For the ave calculation (like tair).
      do j=1,ny
        do i=1,nx
          ave(i,j) = ave(i,j) / ndays
        enddo
      enddo
...
c   Just look at the last 2D array to do this.
      do j=1,ny
        do i=1,nx
          if (var(i,j).eq.undef) ave(i,j) = undef
        enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine find_max (nx,ny,path,irec_start,ndays,undef,
     i  fname,
     o  vmax)
...
c   Just look at the last 2D array to do this.
      do j=1,ny
        do i=1,nx
          if (var(i,j).eq.undef) vmax(i,j) = undef
        enddo
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine find_max_date (nx,ny,path,irec_start,ndays,undef,
     &  doy_start_of_sim,doy_31Dec,
     i  fname,
     o  vmax,vdate_dos,vdate_doy)
      ...
      real var(nx,ny)
      real vmax(nx,ny)
      real vdate_dos(nx,ny)
      real vdate_doy(nx,ny)

      real undef,doy_start_of_sim,doy_31Dec,delta_days
...
c The end day is one less than this start of the next year.
      ...
      print *,nyear,iyr_beg,iyr_end,irec_start,irec_end,ndays
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   ...
   52 format ('TITLE yearly NSIDC-0051 Ice Concentration variables')
